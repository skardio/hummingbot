...
from multi_coin_grid_pro.core.global_risk_manager import CoinCycleState, GlobalRiskManager
...
from multi_coin_grid_pro.core.risk_buckets import BucketExposureTracker, MarketMetrics
...
class MultiCoinGridController(ControllerBase):
    ...
    def __init__(self, config: MultiCoinGridConfig,
        ...
        try:
            ...
            # T5-A1: Injectable clock — defaults to time.time, injectable for backtesting/replay
        ...
        self._regime_macro_trends: Dict[str, Any] = {}
        ...
        self.bot_start_time: float = self._clock()  # Track when bot started (for startup delay)
        ...
        # T4-R4: Warmup gate — no trading until on_start() reconciliation is complete
        self._warmup_complete: bool = False
        self._warmup_steps_done: int = 0
        self._warmup_steps_total: int = 6

        # T4-R5: Open orders snapshot loaded on startup to prevent double exposure
        self._exchange_open_orders: List = []

        # T5-A10: Late instance vars — all declared here to avoid AttributeError landmines
        self._bot_start_time: Optional[float] = None
        self._executor_creation_times: Dict[str, float] = {}
        self._orphaned_positions: List = []
        self._stale_orders: List = []
        self._risk_block_new_entries: bool = False
        self._stale_executor_counts: Dict[str, int] = {}
        self._kill_switch_actions_sent: bool = False
        self._last_memory_cleanup: float = 0.0
        ...
        self.last_successful_api_call: float = self._clock()
        ...
        self._last_regime_input_pair: Optional[str] = None
        self._last_regime_input_source: Optional[str] = None
        ...
        # Phase 1 fee-diagnostic: per-symbol consecutive loss streak tracker
        self._loss_streaks: Dict[str, int] = {}  # {symbol: consecutive_negative_roundtrips}
        ...
        # Item 12/13/14: trade labels feed session checks, meta ranking, and later MC analysis.
        self._trade_labels: list = []  # List[TradeLabel] — loaded on startup and fed on close
        self._entry_context_by_symbol: Dict[str, Dict[str, Any]] = {}
        self.trade_label_store: Optional[TradeLabelStore] = None
        try:
            _label_instance = getattr(self.config, "instance_id", None) or self.config.quote_asset.lower()
            self.trade_label_store = TradeLabelStore(db_path=f"data/trade_labels_{_label_instance}.db")
        except Exception as _tle:
            self.logger().warning(f"⚠️  TradeLabelStore unavailable: {_tle}")
        ...
        if structured_events_enabled:
            try:
                dl_dir = observability_cfg.get('decision_log_dir', 'data/decision_logs') if isinstance(observability_cfg, dict) else getattr(observability_cfg, 'decision_log_dir', 'data/decision_logs')  # noqa: E501
                dl_retention = observability_cfg.get('decision_log_retention_days', 90) if isinstance(observability_cfg, dict) else getattr(observability_cfg, 'decision_log_retention_days', 90)  # noqa: E501
                self.decision_logger = DecisionLogger(
                    enabled=True,
                    output_dir=dl_dir,
                    retention_days=dl_retention,
                )
                self.logger().info(f"✅ AI-F1 DecisionLogger initialized (dir={dl_dir})")
            except Exception as e:
        ...
        self.execution_funnel = ExecutionFunnelTracker(
            window_sec=int(getattr(config, "execution_funnel_window_sec", 900) or 900)
        )
        ...
        self.momentum_candidate_scorer: Optional[MomentumCandidateScorer] = None
        self._momentum_rejected_log_ts: Dict[str, float] = {}
        momentum_cfg = getattr(config, 'momentum_sleeve', {}) or {}
        momentum_mode = self._momentum_config_mode(momentum_cfg)
        if momentum_mode == 'detect_only':
            self.momentum_candidate_scorer = MomentumCandidateScorer(
                self._momentum_config_as_dict(momentum_cfg)
            )
            self.logger().info("📈 MomentumCandidateScorer enabled (detect-only, no orders)")
        else:
            self.logger().info("📈 MomentumCandidateScorer disabled")
        ...
        self._last_cooldown_cleanup: float = self._clock()
        ...
        try:
            instance_id = getattr(config, 'instance_id', None)
            ep_db_path = f"data/entry_prices_{instance_id}.db" if instance_id else "data/entry_prices.db"
            self.entry_price_store = EntryPriceStore(ep_db_path, logger=self.logger())
            self.logger().info(f"✅ T1-K4: EntryPriceStore initialized ({ep_db_path})")
        except Exception as e:
            self.logger().error(f"❌ T1-K4: EntryPriceStore init failed: {e}")
            self.entry_price_store = None
        ...
        self._last_performance_report_time: float = self._clock()
        ...
        self.grid_suitability_scorer = None
        grid_suitability_cfg = getattr(config, 'grid_suitability', None)
        if grid_suitability_cfg and grid_suitability_cfg.get('enabled', False):
            try:
                from multi_coin_grid_pro.utils.grid_suitability_scorer import GridSuitabilityScorer
                self.grid_suitability_scorer = GridSuitabilityScorer(grid_suitability_cfg, self.logger())
                mode_str = "LOGGING ONLY" if grid_suitability_cfg.get('logging_only', True) else "ACTIVE"
                self.logger().info(f"📐 Grid Suitability Scorer: ENABLED ({mode_str})")
                self.logger().info(f"   Min score: {grid_suitability_cfg.get('min_grid_score', 0.30)}")
                self.logger().info(f"   Lookback: {grid_suitability_cfg.get('lookback_candles', 48)} candles")
            except Exception as e:
                self.logger().error(f"❌ Grid Suitability Scorer failed to initialize: {e}")
                self.grid_suitability_scorer = None
    ...
    @property
    def _balance_currency(self) -> str:
        """Return the currency to use for balance lookups.
        OKX EU trades USD pairs (BTC-USD) but wallet holds USDC → set balance_currency: USDC in config.
        """
        return getattr(self.config, "balance_currency", None) or self.config.quote_asset

    def _get_quote_balances_for_allocation(self) -> Tuple[Decimal, Decimal, Decimal]:
        """
        Return raw quote balance, effective quote balance, and internally reserved quote.

        The BudgetAllocator subtracts reservations during check_budget(). Allocation
        should size from effective balance, but budget checks must receive raw balance
        to avoid subtracting the same reservation twice.
        """
        try:
            connector = getattr(self, "connector", None)
            if connector is None:
                connector = self.connectors.get(self.config.connector_name)
            # Use balance_currency if set (e.g. OKX EU: quote_asset=USD but wallet holds USDC)
            balance_ccy = getattr(self.config, "balance_currency", None) or self.config.quote_asset
            raw_available_balance = Decimal(str(connector.get_available_balance(balance_ccy)))
            self.logger().debug(
                f"📊 Available balance for grid: {raw_available_balance} {balance_ccy}"
            )
        except Exception as e:
            self.logger().warning(f"Could not get available balance: {e}, using config")
            raw_available_balance = Decimal(str(self.config.total_amount_quote))

        budget_reserved = self.budget_allocator.total_reserved
        effective_available_balance = max(Decimal("0"), raw_available_balance - budget_reserved)

        if budget_reserved > Decimal("0"):
            self.logger().debug(
                f"📊 Effective balance after reservations: {effective_available_balance} "
                f"(raw: {raw_available_balance}, reserved: {budget_reserved})"
            )

        return raw_available_balance, effective_available_balance, budget_reserved

    def _apply_capital_reserve_to_balance(
        self,
        available_balance: Decimal,
        log_prefix: str = "",
    ) -> Decimal:
        """Apply the configured reserve before dynamic allocation sizes new slots."""
        reserve_pct = float(getattr(self.config, "capital_reserve_pct", 0.0) or 0.0)
        if reserve_pct <= 0:
            return available_balance

        try:
            deployable = self.risk_manager.get_deployable_balance(
                available_balance,
                reserve_pct=reserve_pct,
            )
        except Exception as exc:
            self.logger().warning(f"{log_prefix}⚠️  Capital reserve check failed: {exc}")
            return available_balance

        if deployable < available_balance:
            self.logger().debug(
                f"{log_prefix}💰 Capital reserve: {float(available_balance):.2f}"
                f" → {float(deployable):.2f}"
                f" ({reserve_pct:.0f}% reserved)"
            )
        return deployable

    @staticmethod
    def _early_stop_reason_from_stop_reason(
        stop_reason: Optional[str],
        emergency: bool = False,
    ) -> Optional[EarlyStopReason]:
        reason = (stop_reason or "").lower()
        if reason == "stop_loss":
            return EarlyStopReason.STOP_LOSS
        if reason == "hard_stop_exit":
            return EarlyStopReason.HARD_STOP_EXIT
        if reason == "emergency_exit":
            return EarlyStopReason.EMERGENCY_EXIT
        if reason == "api_error_threshold":
            return EarlyStopReason.RISK_GUARD
        if "switch" in reason:
            return EarlyStopReason.STRATEGY_SWITCH
        if reason in ("time_stop", "time_limit"):
            # TIME_STOP from US-005 (max hold time elapsed) — maps to NO_PROGRESS_TIMEOUT
            # so the executor's fee-aware guard uses its configured timeout-bypass window.
            return EarlyStopReason.NO_PROGRESS_TIMEOUT
        if emergency:
            return EarlyStopReason.EMERGENCY_EXIT
        return None
    ...
    def stop(self):
        ...
        try:
            if hasattr(self, 'decision_logger') and self.decision_logger:
                self.decision_logger.close()
                stats = self.decision_logger.stats
                self.logger().info(
                    f"✅ AI-F1 DecisionLogger closed "
                ...
                if trace.accepted:
                    self.event_logger.emit_gate_passed(
                        ...
                        metadata={
                            ...
                            "regime": getattr(self, "_last_detected_regime", None) or "UNKNOWN",
                        }
                    )
                else:
    ...
    # ── AI-F1: Decision Snapshot Logging ──────────────────────────
    ...
    def _log_entry_decision(
        ...
        try:
            ...
            snap.bot_state = build_bot_state(
                self.bot_start_time,
                self.circuit_breaker_active,
                self.api_error_paused,
    ...
    def _log_rotation_decision(
        ...
        from_trend_pct: float = 0.0,
        to_trend_pct: float = 0.0,
        switch_cost: Optional[float] = None,
    ) -> None:
        """Log a rotation/switch decision snapshot.  Never raises."""
        if not self.decision_logger:
            return
        try:
            snap = self.decision_logger.create_snapshot("rotation", symbol, self.config.connector_name)
            snap.outcome = "accepted" if accepted else "rejected"
            snap.rejected_by = rejected_by
            snap.reason_msg = reason_msg
            snap.market = build_market_features(
            ...
            snap.risk = build_risk_context(self.risk_manager, self.drawdown_tracker)
            snap.rotation = build_rotation_context(
                from_coin=from_coin,
                to_coin=symbol,
    ...
    def _log_selection_trace(
        ...
        try:
            ...
            if self.execution_funnel:
                self.execution_funnel.record_tick()
                self.execution_funnel.record_considered(len(top_coins))
                if best_coin:
                    self.execution_funnel.record_allowed(best_coin)
                elif rejection_reason:
                    self.execution_funnel.record_rejected("NONE", rejection_reason)
                self.execution_funnel.maybe_log_summary()
    ...
    def _apply_universe_quality_gate(self, excluded_coins: set) -> set:
        """ST-02: Exclude low-quality universe members before trend/GridScore ranking."""
        try:
            if not self.trend_calculator:
                return set()

            symbols = set(getattr(self.trend_calculator, "trends", {}).keys())
            symbols.update(getattr(self, "monitored_coins", []) or [])
            symbols.update(getattr(self, "all_available_pairs", []) or [])
            symbols = {s for s in symbols if s and s not in excluded_coins}
            if not symbols:
                return set()

            has_volume_data = bool(getattr(self, "pair_volumes", {}))
            has_spread_data = bool(getattr(self, "pair_spreads", {}))
            candidates = [
                (
                    symbol,
                    float(self.pair_volumes.get(symbol, 0.0)) if has_volume_data else 0.0,
                    float(self.pair_spreads.get(symbol, 999.0)) if has_spread_data else 0.0,
                )
                for symbol in sorted(symbols)
                if symbol.endswith(f"-{self.config.quote_asset}")
            ]

            base_min_volume = (
                self._base_min_volume_threshold()
                if has_volume_data else 0.0
            )
            effective_min_volume = (
                self._resolve_adaptive_min_volume_threshold(
                    pair_volumes={symbol: volume for symbol, volume, _ in candidates},
                    target_count=max(1, int(getattr(self.config, "max_coins_to_monitor", 1))),
                    context="st02_gate",
                )
                if has_volume_data else 0.0
            )
            ...
            cfg = QualityGateConfig(
                enabled=True,
                extra_blacklist=set(getattr(self.config, "blacklist", []) or []),
                min_24h_volume=effective_min_volume,
                max_spread_pct=(
                    float(getattr(self.config, "max_spread_pct", 0.0))
                    if has_spread_data else 0.0
                ),
            )
            ...
            if result.rejected:
                stats = ", ".join(f"{reason}={count}" for reason, count in sorted(result.stats.items()))
                self.logger().info(
                    f"🧹 ST-02 universe gate: {len(result.rejected)}/{len(candidates)} "
                    f"filtered before ranking ({stats})"
                )
            if has_volume_data and effective_min_volume < base_min_volume:
                self.logger().info(
                    f"🛟 ST-02 adaptive liquidity floor: min volume relaxed "
                    f"from €{base_min_volume:,.0f} to €{effective_min_volume:,.0f}"
                )
            return set(result.rejected)
        except Exception as exc:
            self.logger().warning(f"⚠️  ST-02 universe quality gate failed: {exc}")
            return set()

    def _base_min_volume_threshold(self) -> float:
        """Return configured base 24h volume threshold in quote currency."""
        return float(getattr(self.config, 'min_24h_volume_usdt', self.config.min_24h_volume_eur))

    def _resolve_adaptive_min_volume_threshold(self, pair_volumes: Dict[str, float], target_count: int,
                                               context: str) -> float:
        """
        Compute an adaptive minimum volume threshold to avoid universe starvation.

        The threshold never increases above the configured base and never relaxes
        below a configurable floor ratio of the base.
        """
        base_threshold = self._base_min_volume_threshold()
        if base_threshold <= 0 or not pair_volumes:
            return base_threshold

        if not getattr(self.config, "universe_adaptive_liquidity_enabled", True):
            return base_threshold

        desired = max(1, int(target_count))
        eligible_at_base = sum(1 for volume in pair_volumes.values() if float(volume) >= base_threshold)
        if eligible_at_base >= desired:
            return base_threshold

        floor_ratio = float(getattr(self.config, "universe_adaptive_min_volume_floor_ratio", 0.25))
        floor_ratio = min(max(floor_ratio, 0.01), 1.0)
        floor_threshold = base_threshold * floor_ratio

        ranked_volumes = sorted((float(v) for v in pair_volumes.values() if float(v) > 0), reverse=True)
        if not ranked_volumes:
            return base_threshold

        nth_index = min(desired - 1, len(ranked_volumes) - 1)
        target_threshold = ranked_volumes[nth_index]
        effective_threshold = max(floor_threshold, min(base_threshold, target_threshold))

        if effective_threshold < base_threshold:
            eligible_effective = sum(1 for volume in pair_volumes.values() if float(volume) >= effective_threshold)
            self.logger().info(
                f"🛟 Adaptive universe threshold ({context}): {eligible_at_base}->{eligible_effective} "
                f"eligible coins, min volume €{base_threshold:,.0f} -> €{effective_threshold:,.0f}"
            )

        return effective_threshold

    def _market_metrics_for(self, symbol: str) -> Optional[MarketMetrics]:
        """Build bucket-classification market metrics from cached volume/spread data."""
        vol = getattr(self, "pair_volumes", {}).get(symbol)
        spr = getattr(self, "pair_spreads", {}).get(symbol)
        if vol is None or spr is None:
            return None
        return MarketMetrics(volume_24h_usd=float(vol), spread_pct=float(spr) * 100.0)

    def _orderbook_depth_multiple(self, symbol: str, order_size_quote: Decimal) -> float:
        """Return min(bid_depth, ask_depth) / order size using the cached order book."""
        try:
            size = float(order_size_quote)
            if size <= 0 or not self.connector:
                return 1.0
            order_book = self.connector.get_order_book(symbol)
            if not order_book or not getattr(order_book, "snapshot", None):
                return 1.0
            ...
            def _side_depth(levels) -> float:
                total = 0.0
                if hasattr(levels, "iloc"):
                    iterable = ((levels.iloc[i, 0], levels.iloc[i, 1]) for i in range(len(levels)))
                else:
                    iterable = levels
                for price, amount in iterable:
                    total += float(price) * float(amount)
                    if total >= size * 20.0:
                        break
                return total

            return max(0.0, min(_side_depth(bids), _side_depth(asks)) / size)
        except Exception:
            return 1.0

    def _btc_1h_pct(self) -> float:
        """Best-effort BTC 1h trend for quality scoring."""
        try:
            if not self.trend_calculator:
                return 0.0
            btc_trend = self.trend_calculator.get_trend(f"BTC-{self.config.quote_asset}")
            if not btc_trend:
                return 0.0
            return float(getattr(btc_trend, "trend_60m", 0.0) or 0.0)
        except Exception:
            return 0.0

    def _score_trade_quality(self, symbol: str, requested_notional: Decimal):
        """Compute a less-blind TradeQualityScorer input set from cached market data."""
        trend = self.trend_calculator.get_trend(symbol) if self.trend_calculator else None
        indicators = self._calculate_smart_entry_indicators(symbol, trend) if trend else None

        rsi = indicators.rsi_14 if indicators else 50.0
        atr_pct = indicators.atr_pct if indicators and indicators.atr_pct > 0 else 0.0
        if atr_pct <= 0 and trend:
            vol = float(getattr(trend, "volatility", 0.0) or 0.0)
            atr_pct = vol * 100.0 if 0.0 < vol < 0.25 else vol

        raw_spread = float(getattr(self, "pair_spreads", {}).get(symbol, 0.005) or 0.0)
        spread_pct = raw_spread * 100.0 if raw_spread <= 1.0 else raw_spread
        depth_multiple = self._orderbook_depth_multiple(symbol, requested_notional)
        ...
        score = self.quality_scorer.score(
            ...
            atr_pct=atr_pct,
            btc_1h_pct=self._btc_1h_pct(),
        )
        self._quality_score = score.total
        self.logger().info(
            f"🎯 Quality score {symbol}: {score.total:.0f}/100 "
            f"(rsi={rsi:.1f} spread={spread_pct:.3f}% "
            f"depth={depth_multiple:.1f}x atr={atr_pct:.2f}%)"
        )
        return score, depth_multiple, spread_pct

    def _entry_cycle_number(self, symbol: str, close_ts: int) -> int:
        day = datetime.utcfromtimestamp(close_ts).date()
        return 1 + sum(
            1
            for label in self._trade_labels
            if label.coin == symbol and datetime.utcfromtimestamp(label.timestamp_close).date() == day
        )

    def _is_reentry_same_session(self, symbol: str, session: str) -> bool:
        return any(label.coin == symbol and label.session == session for label in self._trade_labels[-50:])

    def _evaluate_entry_admission(
        self,
        *,
        symbol: str,
        ...
        """Shared gate stack for the first coin and all multi-coin follow-up slots."""
        prefix = log_prefix

        try:
            cycle_state = self.risk_manager.get_coin_cycle_state(symbol)
            if cycle_state == CoinCycleState.TWO_FAILED_CYCLES:
                failed_cycles = self.risk_manager.get_failed_cycles(symbol)
                self.logger().info(
                    f"{prefix}🔒 Entry blocked {symbol}: "
                    f"state={cycle_state.value} failed_cycles={failed_cycles}"
                )
                return False, requested_notional
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Cycle-state check failed: {exc}")

        try:
            cur_session = session_from_utc(now_ts)
            if self.session_edge_detector.should_avoid_session(
                session=cur_session,
                labels=self._trade_labels,
                coin=symbol,
            ):
                self.logger().info(f"{prefix}⏰ Session edge: avoiding {symbol} in {cur_session}")
                return False, requested_notional
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Session edge check failed: {exc}")

        try:
            _quality, depth_multiple, spread_pct = self._score_trade_quality(symbol, requested_notional)
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Quality scorer failed: {exc}")
            depth_multiple = 1.0
            spread_pct = float(getattr(self, "pair_spreads", {}).get(symbol, 0.0) or 0.0)
            self._quality_score = 50.0

        try:
            decision = self.entry_gateway.can_enter(
                symbol=symbol,
            ...
            if not decision.allowed:
                self.logger().info(f"{prefix}🚪 Entry gateway blocked {symbol}: {decision.reason}")
                return False, requested_notional
            adjusted_notional = decision.approved_notional or requested_notional
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Entry gateway failed: {exc}")
            adjusted_notional = requested_notional

        try:
            halved = (
                bool(getattr(self.config, "daily_kill_switch_enabled", True))
                and self.risk_manager.is_coin_halved_today(symbol)
            )
            q_mult = quality_size_multiplier(self._quality_score, halved=halved)
            if q_mult != 1.0:
                before = adjusted_notional
                adjusted_notional = adjusted_notional * Decimal(str(q_mult))
                self.logger().info(
                    f"{prefix}📊 Quality sizing {symbol}: "
                    f"score={self._quality_score:.0f} → {q_mult:.2f}x "
                    f"({float(before):.2f}→{float(adjusted_notional):.2f})"
                )
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Quality sizing failed: {exc}")

        try:
            decision = self.entry_gateway.can_enter(
                symbol=symbol,
                requested_notional=adjusted_notional,
                now=now_ts,
                quality_score=int(self._quality_score),
                logger=self.logger(),
            )
            if not decision.allowed:
                self.logger().info(f"{prefix}🚪 Entry gateway blocked resized {symbol}: {decision.reason}")
                return False, adjusted_notional
            adjusted_notional = decision.approved_notional or adjusted_notional
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Entry gateway resize check failed: {exc}")

        try:
            bucket_ok, bucket_reason = self.bucket_tracker.can_add(
                symbol,
                float(adjusted_notional),
                metrics=self._market_metrics_for(symbol),
            )
            if not bucket_ok:
                self.logger().info(f"{prefix}🪣 Bucket cap {symbol}: {bucket_reason}")
                return False, adjusted_notional
        except Exception as exc:
            self.logger().warning(f"{prefix}⚠️  Bucket tracker failed: {exc}")

        self._entry_context_by_symbol[symbol] = {
            "timestamp_open": int(now_ts),
            "session": session_from_utc(now_ts),
            "regime": self._last_detected_regime or "",
            "spread_at_entry": spread_pct,
            "depth_at_entry": depth_multiple,
            "quality_score": int(self._quality_score),
        }
        try:
            self.execution_funnel.record_approved(symbol)
        except Exception:
            pass
        return True, adjusted_notional
    ...
    def _handle_parabolic_cooldown(self, symbol: str, reason: str):
        ...
        try:
            ...
            expiry_ts = self._clock() + cooldown_sec
    ...
    def _calculate_portfolio_value(self) -> Decimal:
        ...
        """
        try:
            quote_balance = self.connector.get_balance(self._balance_currency)
            ...
            if coin_value_total > 0:
                self.logger().debug(
                    ...
                    f"= {total_value:.2f} {self._balance_currency}"
                )
            ...
            return self.connector.get_balance(self._balance_currency)
    ...
    def _get_current_max_slots(self) -> int:
        ...
        if not self.dynamic_slot_manager.enabled:
            self.logger().info(
                f"[REGIME_ROUTE] source=static detected=None normalized=static "
                f"slots={self.max_simultaneous_coins} balance=n/a"
            )
        ...
        try:
            ...
            cached_regime = getattr(self, "_last_detected_regime", None)
            self.logger().info(
                f"[REGIME_ROUTE] source=portfolio_error detected={cached_regime or 'None'} "
                f"normalized={cached_regime or 'baseline'} slots={self.max_simultaneous_coins} balance=n/a"
            )
            return self.max_simultaneous_coins

        # Get current regime
        regime_source = "baseline_fallback"
        ...
        try:
            ...
            if self.market_regime_filter:
                ...
                if regime_state:
                    raw_regime = getattr(regime_state, "regime", None)
                    if raw_regime:
                        current_regime = str(raw_regime).upper()
                    elif hasattr(regime_state, "is_favorable"):
                        current_regime = "BULL" if regime_state.is_favorable else "BEAR"
                    detected_regime = current_regime
                    regime_source = "market_regime_filter"

            if not current_regime:
                cached_regime = getattr(self, "_last_detected_regime", None)
                current_regime = cached_regime or "baseline"
                detected_regime = cached_regime
                regime_source = "adaptive_cached" if cached_regime else "baseline_fallback"
        except Exception as e:
            self.logger().warning(f"Failed to get regime for dynamic slots: {e}, using cached/baseline")
            cached_regime = getattr(self, "_last_detected_regime", None)
            current_regime = cached_regime or "baseline"
            detected_regime = cached_regime
            regime_source = "adaptive_cached" if cached_regime else "baseline_fallback"
        ...
        self.logger().info(
            f"[REGIME_ROUTE] source={regime_source} detected={detected_regime or 'None'} "
            f"normalized={current_regime} slots={dynamic_slots} balance={account_balance}"
        )

        return dynamic_slots

    def _get_dynamic_slots_count(self) -> int:
        """Compatibility alias for dynamic slot routing tests."""
        return self._get_current_max_slots()
            ...
            if not self.api_error_paused:
                ...
                if self.active_coin and self.active_executor_id and self._is_executor_actually_active():
                    ...
                    stop_action = self._create_stop_action(
                        emergency=True,
                        stop_reason="api_error_threshold",
                    )
                    if stop_action:
    ...
    async def _api_call_with_error_handling(self, func, *args, **kwargs) -> Optional[any]:
        ...
        if self.api_error_paused:
            ...
            time_since_pause = self._clock() - (self.api_error_paused_at or 0)
        ...
        try:
            ...
            if self.consecutive_api_errors > 0:
                time_since_last_error = self._clock() - self.last_api_error_time
            ...
            self.last_successful_api_call = self._clock()
    ...
    async def _get_ticker_data_safe(self) -> Dict[str, Any]:
        ...
        if 'bitget' in connector_name:
            ...
            self._last_ticker_call_time = self._clock()
            return ticker_data

        # OKX-specific ticker fetcher
        if 'okx' in connector_name:
            ticker_data = await self._get_okx_ticker_data()
            self._last_ticker_call_time = self._clock()
            return ticker_data

        # Bybit-specific ticker fetcher
        if 'bybit' in connector_name:
            ticker_data = await self._get_bybit_ticker_data()
            self._last_ticker_call_time = self._clock()
            return ticker_data
    ...
    async def _get_okx_ticker_data(self) -> Dict[str, Any]:
        """
        Fetch ticker data from OKX's public API.

        OKX API: GET https://www.okx.com/api/v5/market/tickers?instType=SPOT
        Returns 24h volume, bid/ask prices, last price for all SPOT instruments.
        No authentication required.

        Returns:
            Dict keyed by Hummingbot symbol (e.g., "BTC-USDT") with volume/spread data
        """
        try:
            import aiohttp
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status != 200:
                        self.logger().warning(f"⚠️  OKX ticker API returned status {response.status}")
                        return {}

                    data = await response.json()

                    if data.get('code') != '0':
                        self.logger().warning(f"⚠️  OKX ticker API error: {data.get('msg')}")
                        return {}
                    ...
                    for ticker in tickers:
                        ...
                        # vol24h = base volume, volCcy24h = quote volume
                        quote_vol = float(ticker.get('volCcy24h', 0) or 0)
                        base_vol = float(ticker.get('vol24h', 0) or 0)
                    ...
                    self.logger().info(f"✅ OKX ticker data: {len(result)} pairs with volume data")
                    self.consecutive_api_errors = 0
                    self.last_successful_api_call = self._clock()
                    return result

        except asyncio.TimeoutError:
            self.logger().warning("⚠️  OKX ticker API timeout")
            return {}
        except Exception as e:
            self.logger().warning(f"⚠️  OKX ticker API error: {e}")
            return {}
    ...
    async def _get_bybit_ticker_data(self) -> Dict[str, Any]:
        """
        Fetch ticker data from Bybit's public API.

        Bybit API: GET https://api.bybit.com/v5/market/tickers?category=spot
        Returns 24h volume, bid/ask prices, last price for all SPOT pairs.
        No authentication required.

        Bybit symbol format: "BTCUSDT" (no dash) → converted to "BTC-USDT".
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status != 200:
                        self.logger().warning(f"⚠️  Bybit ticker API returned status {response.status}")
                        return {}

                    data = await response.json()

                    if data.get('retCode') != 0:
                        self.logger().warning(f"⚠️  Bybit ticker API error: {data.get('retMsg')}")
                        return {}
                    ...
                    for ticker in tickers:
                        ...
                        # Convert Bybit symbol to Hummingbot format
                        if symbol.endswith('USDT'):
                            hb_symbol = symbol[:-4] + '-USDT'
                        elif symbol.endswith('USDC'):
                            hb_symbol = symbol[:-4] + '-USDC'
                        elif symbol.endswith('BTC'):
                            hb_symbol = symbol[:-3] + '-BTC'
                        else:
                            continue  # Skip unknown quote assets
                        ...
                        # turnover24h = quote volume (USDT), volume24h = base volume
                        quote_vol = float(ticker.get('turnover24h', 0) or 0)
                        base_vol = float(ticker.get('volume24h', 0) or 0)
                    ...
                    self.logger().info(f"✅ Bybit ticker data: {len(result)} pairs with volume data")
                    self.consecutive_api_errors = 0
                    self.last_successful_api_call = self._clock()
                    return result

        except asyncio.TimeoutError:
            self.logger().warning("⚠️  Bybit ticker API timeout")
            return {}
        except Exception as e:
            self.logger().warning(f"⚠️  Bybit ticker API error: {e}")
            return {}
    ...
    def _initialize_components(self):
        ...
        if not self.trend_calculator:
            ...
            self.trend_calculator = TrendCalculator(
                ...
                warmup_max_minutes=getattr(self.config, 'warmup_max_minutes', 120)
            )
    ...
    async def on_start(self):
        ...
        # T4-R4 warmup step 1/6: Load active cooldowns
        self.logger().info(f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: loading cooldowns")
            ...
            try:
                ...
                if active_cooldowns:
                    ...
                    for symbol, expiry_ts in active_cooldowns.items():
                        remaining = int(expiry_ts - self._clock())
        ...
        # T4-R4 warmup step 2/6: Load persisted entry prices
        self.logger().info(f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: loading entry prices")
        if self.entry_price_store:
            try:
                persisted_prices = self.entry_price_store.load_all()
                if persisted_prices:
                    self.entry_prices.update(persisted_prices)
                    self.logger().info(
                        f"✅ T1-K4: Loaded {len(persisted_prices)} entry prices from database"
                    )
                    for symbol, price in persisted_prices.items():
                        self.logger().info(f"   - {symbol}: €{price:.4f}")
                else:
                    self.logger().info("ℹ️  T1-K4: No persisted entry prices to restore")
            except Exception as e:
                self.logger().error(f"❌ T1-K4: Failed to load entry prices: {e}")
        self._warmup_steps_done += 1

        # T4-R4 warmup step 3/6: Reconciling fills (trade labels)
        self.logger().info(f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: reconciling fills")
        if self.trade_label_store:
            try:
                labels = self.trade_label_store.query_recent(days=30)
                if labels:
                    self._trade_labels = list(reversed(labels))[-500:]
                    self.logger().info(
                        f"✅ ST-06b: Loaded {len(self._trade_labels)} trade labels from persistence"
                    )
                else:
                    self.logger().info("ℹ️  ST-06b: No persisted trade labels to restore")
            except Exception as e:
                self.logger().error(f"❌ ST-06b: Failed to load trade labels: {e}")
        self._warmup_steps_done += 1

        # T4-R4 warmup step 4/6: Detecting orphaned positions + restoring bucket state
        self.logger().info(
            f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: detecting orphaned positions"
        )
        self._restore_bucket_tracker_state()
        await self._detect_orphaned_positions()
        self._warmup_steps_done += 1

        # T4-R4 warmup step 5/6: Load open orders from exchange + external fill reconciliation
        self.logger().info(f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: loading open orders")
        await self._load_exchange_open_orders()
        await self._reconcile_external_fills()
        self._warmup_steps_done += 1

        # T4-R4 warmup step 6/6: Verify risk module / super().on_start()
        self.logger().info(f"warmup {self._warmup_steps_done}/{self._warmup_steps_total}: verifying risk module")
        await super().on_start()
        self._warmup_steps_done += 1

        # T4-R4: All reconciliation steps complete — release the trading gate
        self._warmup_complete = True
        self.logger().info(
            f"✅ T4-R4: Warmup complete ({self._warmup_steps_done}/{self._warmup_steps_total})"
    ...
    async def _load_exchange_open_orders(self) -> None:
        """
        T4-R5: Load open orders from exchange on startup.

        Populates self._exchange_open_orders with InFlightOrder objects currently
        tracked by the connector.  This snapshot lets the controller reconcile
        its own executor state against real exchange state and avoids placing
        duplicate orders for positions that already have live orders.
        """
        self._exchange_open_orders = []

        if not self.connector:
            self.logger().debug("T4-R5: No connector available — skipping open order load")
            return

        try:
            in_flight = getattr(self.connector, 'in_flight_orders', {})
            if in_flight:
                self._exchange_open_orders = list(in_flight.values())
                self.logger().info(
                    f"✅ T4-R5: Loaded {len(self._exchange_open_orders)} open order(s) from exchange"
                )
                for order in self._exchange_open_orders:
                    pair = getattr(order, 'trading_pair', '?')
                    side = getattr(order, 'trade_type', '?')
                    price = getattr(order, 'price', '?')
                    amount = getattr(order, 'amount', '?')
                    self.logger().info(f"   - {pair} {side} price={price} amount={amount}")
            else:
                self.logger().info("ℹ️  T4-R5: No open orders on exchange at startup")
        except Exception as e:
            self.logger().error(f"❌ T4-R5: Failed to load open orders: {e}")
    ...
    async def _detect_orphaned_positions(self):
        ...
        try:
            # Wait for connector to initialize balances — retry up to 30s
            for _wait in range(6):
                await asyncio.sleep(5)
                if self.connector and hasattr(self.connector, '_account_balances'):
                    break
            else:
                self.logger().warning("⚠️ Orphan detection: Connector not ready after 30s — skipping")
                return
            ...
            # ExecutorInfo is a Pydantic model — use .trading_pair property directly
            active_executor_coins = set()
            if hasattr(self, 'executors_info') and self.executors_info:
                for executor_info in self.executors_info:
                    if getattr(executor_info, 'is_active', False):
                        tp = getattr(executor_info, 'trading_pair', None)
                        if tp:
                            active_executor_coins.add(tp.split('-')[0])

            # Also check active_coins dict
            for coin in self.active_coins.keys():
                active_executor_coins.add(coin.split('-')[0])

            tracked_orphan_base_by_asset: Dict[str, Decimal] = {}
            if hasattr(self, 'executors_info') and self.executors_info:
                for executor_info in self.executors_info:
                    if getattr(executor_info, 'is_active', False):
                        continue
                    ...
                    if not tp:
                        continue
                    tracked_base = self._tracked_orphan_base_from_custom_info(
                        getattr(executor_info, 'custom_info', {}) or {}
                    )
                    if tracked_base <= Decimal("0"):
                        continue
                    asset = tp.split('-')[0]
                    tracked_orphan_base_by_asset[asset] = (
                        tracked_orphan_base_by_asset.get(asset, Decimal("0")) + tracked_base
                    )
            ...
            for asset, amount in balances.items():
                ...
                try:
                    ...
                    tracked_base = tracked_orphan_base_by_asset.get(asset)
                    orphaned_positions.append({
                        ...
                        'max_sell_amount': float(tracked_base) if tracked_base and tracked_base > Decimal("0") else None,
                    })
            ...
            if orphaned_positions:
                ...
                # === AUTO-SELL ORPHANED POSITIONS ===
                auto_sell = getattr(self.config, 'auto_sell_orphaned_positions', False)
                if auto_sell:
                    capped_positions = [
                        pos for pos in orphaned_positions
                        if pos.get('max_sell_amount') is not None and pos.get('max_sell_amount') > 0
                    ]
                    uncapped_positions = [
                        pos for pos in orphaned_positions
                        if pos.get('max_sell_amount') is None or pos.get('max_sell_amount') <= 0
                    ]
                    if capped_positions:
                        await self._sell_orphaned_positions(capped_positions)
                    if uncapped_positions:
                        self.logger().error(
                            f"❌ ORPHAN-SELL: Refusing to auto-sell {len(uncapped_positions)} startup "
                            f"orphan(s) without bot-tracked base cap"
                        )
                        if hasattr(self, 'telegram_alerter') and self.telegram_alerter.enabled:
                            unsafe_lines = ["<b>🔴 ORPHAN AUTO-SELL BLOCKED</b>\n"]
                            for pos in uncapped_positions:
                                unsafe_lines.append(
                                    f"• <b>{pos['asset']}</b>: no bot-tracked amount, manual review required"
                                )
                            self.telegram_alerter.warning("\n".join(unsafe_lines))
                else:
                    self.logger().warning(
                        "⚠️ auto_sell_orphaned_positions is disabled — skipping auto-sell"
                    )
                    if hasattr(self, 'telegram_alerter') and self.telegram_alerter.enabled:
                        self.telegram_alerter.warning("\n".join(orphan_lines) + "\n\n⚠️ Auto-sell disabled")
            else:
    ...
    @staticmethod
    def _tracked_orphan_base_from_custom_info(custom_info: Dict[str, Any]) -> Decimal:
        """Return bot-tracked remaining base inventory from executor custom_info."""
        ...
        position_base = _as_decimal(custom_info.get("position_size_base"))
        if position_base > Decimal("0"):
            return position_base

        break_even = _as_decimal(custom_info.get("break_even_price"))
        position_quote = _as_decimal(custom_info.get("position_size_quote"))
        if position_quote > Decimal("0") and break_even > Decimal("0"):
            return position_quote / break_even

        buy_quote = _as_decimal(custom_info.get("realized_buy_size_quote"))
        sell_quote = _as_decimal(custom_info.get("realized_sell_size_quote"))
        unsold_quote = max(Decimal("0"), buy_quote - sell_quote)
        if unsold_quote > Decimal("0") and break_even > Decimal("0"):
            return unsold_quote / break_even

        return Decimal("0")

    async def _check_single_orphan(
        self,
        trading_pair: str,
        max_sell_quote: Optional[Decimal] = None,
        max_sell_amount: Optional[Decimal] = None,
        require_sell_cap: bool = False,
    ) -> None:
        """
        Runtime orphan check for a single pair, triggered when its executor closes as FAILED.

        Waits a few seconds for balance to settle, then checks if the bot still holds
        coins for that pair without an active executor. If so, calls _sell_orphaned_positions.

        Args:
            trading_pair:   e.g. "STRK-USD"
            max_sell_quote: Legacy bot-tracked unsold amount in quote (USD), converted
                            to base with the current mid price when max_sell_amount is absent.
            max_sell_amount: Bot-tracked unsold base amount. Prefer this over max_sell_quote.
            require_sell_cap: If True, never auto-sell without a bot-tracked base cap.
        """
        import asyncio
        await asyncio.sleep(10)  # Let balance updates propagate

        try:
            if not self.connector or not hasattr(self.connector, '_account_balances'):
                return

            base_asset = trading_pair.split('-')[0]
            amount = self.connector._account_balances.get(base_asset, Decimal("0"))
            if amount <= Decimal("0.0001"):
                self.logger().info(f"✅ ORPHAN-CHECK: {base_asset} balance=0 — no orphan")
                return

            # Confirm no active executor is running for this pair
            for ei in self.executors_info:
                if getattr(ei, 'is_active', False):
                    tp = getattr(ei, 'trading_pair', None)
                    if tp == trading_pair:
                        self.logger().info(f"✅ ORPHAN-CHECK: {trading_pair} has active executor — skip")
                        return

            mid_price = self.connector.get_mid_price(trading_pair)
            if not mid_price or mid_price <= Decimal("0"):
                return
            notional = float(amount * mid_price)
            if notional < float(self.config.min_notional):
                return

            # Cap sell to bot-tracked base amount to protect manually-held coins.
            # max_sell_quote is kept as fallback for older callers, but runtime
            # recovery should pass max_sell_amount derived from executor state.
            capped_sell_amount: Optional[float] = None
            if max_sell_amount is not None and max_sell_amount > Decimal("0"):
                capped_sell_amount = float(max_sell_amount)
            elif max_sell_quote is not None and max_sell_quote > Decimal("0"):
                capped_sell_amount = float(max_sell_quote / mid_price)

            if require_sell_cap and capped_sell_amount is None:
                self.logger().error(
                    f"❌ ORPHAN-RUNTIME: {base_asset} orphan detected but bot-tracked "
                    f"base amount is unavailable — refusing blind wallet sell"
                )
                return

            self.logger().warning(
                f"🔴 ORPHAN-RUNTIME: {base_asset} {float(amount):.6f} @ ${float(mid_price):.4f} "
                f"= ${notional:.2f} — no executor, auto-selling"
                + (f" (capped to {capped_sell_amount:.6f} bot-tracked base)" if capped_sell_amount else "")
            )
            auto_sell = getattr(self.config, 'auto_sell_orphaned_positions', False)
            if auto_sell:
                await self._sell_orphaned_positions([{
                    ...
                    'price': float(mid_price),
                    'notional': notional,
                    'max_sell_amount': capped_sell_amount,  # None = sell full wallet balance
                }])
            else:
                self.logger().warning(
                    f"⚠️ ORPHAN-RUNTIME: auto_sell_orphaned_positions disabled — {base_asset} not auto-sold"
                )
                if hasattr(self, 'telegram_alerter') and self.telegram_alerter.enabled:
                    self.telegram_alerter.warning(
                        f"<b>🔴 ORPHANED POSITION (runtime)</b>\n"
                        f"• <b>{base_asset}</b>: {float(amount):.6f} @ ${float(mid_price):.4f} = ${notional:.2f}\n"
                        f"⚠️ auto_sell disabled — sell manually!"
                    )
        except Exception as e:
            self.logger().error(f"❌ _check_single_orphan({trading_pair}) failed: {e}")

    async def _sell_orphaned_positions(self, orphaned_positions: list):
        """
        Auto-sell orphaned positions via market orders.

        Called during startup after detecting coins in wallet without active executors.
        Cancels any open orders for the orphaned pair first (to unlock balance),
        waits briefly, then places a market sell for the full balance.
        """
        import asyncio
        ...
        for pos in orphaned_positions:
            ...
            try:
                # Step 1: Cancel any open orders for this pair to unlock balance
                if hasattr(self.connector, '_in_flight_orders'):
                    for order_id, order in list(self.connector._in_flight_orders.items()):
                        if hasattr(order, 'trading_pair') and order.trading_pair == trading_pair:
                            self.logger().info(
                                f"🧹 ORPHAN-SELL: Canceling stale order {order_id} for {trading_pair}"
                            )
                            try:
                                self.connector.cancel(trading_pair, order_id)
                            except Exception as cancel_err:
                                self.logger().warning(
                                    f"⚠️ Failed to cancel order {order_id}: {cancel_err}"
                                )

                # Step 2: Wait for cancel to settle and balance to unlock
                await asyncio.sleep(3)

                # Step 3: Get TOTAL balance (not available — available may be 0 if locked)
                balance = self.connector.get_balance(asset)
                if balance is None or balance <= Decimal("0"):
                    self.logger().info(f"ℹ️ ORPHAN-SELL: {asset} balance is 0 after cancel — already sold")
                    sold.append(asset)
                    continue

                # Safety cap: never sell more than the bot-tracked unsold amount.
                # This prevents selling manually-held coins on the same account.
                max_sell = pos.get('max_sell_amount')
                if max_sell is not None and max_sell > 0:
                    capped = min(balance, Decimal(str(max_sell)))
                    if capped < balance:
                        self.logger().info(
                            f"🔒 ORPHAN-SELL: {asset} wallet={float(balance):.6f} "
                            f"capped to bot-tracked {float(capped):.6f}"
                        )
                    balance = capped
                ...
                if not trading_rule:
                    failed.append((asset, "no trading rules"))
                    self.logger().warning(f"⚠️ ORPHAN-SELL: No trading rules for {trading_pair}")
                    continue

                min_order_size = trading_rule.min_order_size
                if balance < min_order_size:
                    self.logger().info(
                        f"ℹ️ ORPHAN-SELL: {asset} balance {balance} < min_order_size {min_order_size} — dust"
                    )
                    sold.append(asset)  # Count as resolved (dust)
                    continue

                # Quantize the order amount per exchange rules
                sell_amount = self.connector.quantize_order_amount(trading_pair, balance)
                if sell_amount <= Decimal("0"):
                    self.logger().info(f"ℹ️ ORPHAN-SELL: {asset} quantized to 0 — dust")
                    sold.append(asset)
                    continue

                mid_price = self.connector.get_mid_price(trading_pair)
                if not mid_price or mid_price <= Decimal("0"):
                    failed.append((asset, "no price data"))
                    self.logger().warning(f"⚠️ ORPHAN-SELL: No price data for {trading_pair}")
                    continue

                # Step 5: Place market sell order
                self.logger().warning(
                    f"🔄 ORPHAN-SELL: Selling {sell_amount} {asset} @ market "
                    f"(~${float(sell_amount * mid_price):.2f})"
                )
                ...
                self.logger().warning(
                    f"✅ ORPHAN-SELL: Market sell placed for {sell_amount} {asset} "
                    f"(order_id={order_id})"
                )
                sold.append(asset)
                ...
                self.logger().error(f"❌ ORPHAN-SELL: Failed to sell {asset}: {e}")

        # Summary
        if sold:
            self.logger().warning(
                f"✅ ORPHAN-SELL COMPLETE: Sold {len(sold)}/{len(orphaned_positions)} "
                f"orphaned positions: {sold}"
            )
        if failed:
            self.logger().error(
                f"❌ ORPHAN-SELL FAILURES: {len(failed)}/{len(orphaned_positions)} "
                f"failed: {failed}"
            )

        # Send Telegram summary
        if hasattr(self, 'telegram_alerter') and self.telegram_alerter.enabled:
            lines = ["<b>🔄 ORPHAN AUTO-SELL RESULTS</b>\n"]
            for asset in sold:
                lines.append(f"✅ {asset}: sold")
    ...
    async def _cleanup_stale_orders(self):
        ...
        try:
            ...
            if stale_orders:
                ...
                # T2-O1: Auto-cancel stale orders
                auto_cancel = getattr(self.config, 'auto_cancel_stale_orders', True)
                cancelled = []
                for stale in stale_orders:
                    if auto_cancel:
                        try:
                            ...
                            self.logger().info(
                                f"🗑️ T2-O1: Auto-cancelled stale order "
                                f"{stale['order_id']} on {stale['trading_pair']}"
                            )
                        except Exception as cancel_err:
                            self.logger().error(
                                f"❌ T2-O1: Failed to cancel stale order "
                                f"{stale['order_id']}: {cancel_err}"
                            )
                ...
                if hasattr(self, 'telegram_alerter') and self.telegram_alerter.enabled:
                    ...
                    if cancelled:
                        stale_lines.append(f"\n✅ Auto-cancelled {len(cancelled)}/{len(stale_orders)} orders")
                    else:
                        stale_lines.append("\n⚠️ Auto-cancel disabled - cancel manually")
                    self.telegram_alerter.warning("\n".join(stale_lines))
    ...
    async def control_task(self):
        ...
        try:
            ...
            await self._kill_switch_stop_all_executors()
            return  # T0-Q6: Fail closed — block trading if risk guard errors
        ...
        if (current_time - getattr(self, '_last_memory_cleanup', 0)) >= 300:  # 5 minutes
            try:
                ...
                if hasattr(self, 'trend_calculator') and self.trend_calculator and hasattr(self.trend_calculator, 'trends'):
                    self._cache_regime_macro_trends(remove_from_grid_trends=False)
                ...
                # 6. Garbage collection removed (T0-Q4: gc.collect() in hot path causes 50-200ms STW pause per tick)
        ...
        if hasattr(self, 'regime_detector') and self.regime_detector:
            try:
                ...
                if regime_state:
                    ...
                    if regime_changed:
                        source = getattr(self, "_last_regime_input_source", None) or "unknown"
                        pair = getattr(self, "_last_regime_input_pair", None) or "unknown"
                        if self._last_logged_regime is None:
                            self.logger().info(
                                f"🌡️ REGIME (initial boot): {accepted_regime} "
                                f"(score: {regime_state.score:.1f}, "
                                f"confidence: {regime_state.confidence:.2f}, "
                                f"duration: {regime_state.duration_minutes}m, "
                                f"source: {source}/{pair})\n"
        ...
        if self._console_reporter and self._report_interval_minutes > 0:
            ...
            if time_since_last_report >= report_interval:
                try:
                    ...
                    self.logger().info("=" * 80)
                    if hasattr(self, 'event_logger') and self.event_logger and self.event_logger.enabled:
    ...
    async def update_processed_data(self):
        ...
        if not self.monitored_coins:
            ...
            if not use_dynamic and manual_pairs and isinstance(manual_pairs, list) and len(manual_pairs) > 0:
                    ...
                    if not self.trend_calculator._historical_data_loaded:
                        ...
                        try:
                            ...
                            try:
                                ...
                                await self._ensure_regime_macro_trends(load_history=True, update_prices=True)
                ...
                try:
                    ...
                    base_min_volume = self._base_min_volume_threshold() if pair_volumes else 0.0
                    min_volume = (
                        self._resolve_adaptive_min_volume_threshold(
                            pair_volumes=pair_volumes,
                            target_count=max(1, int(max_coins)),
                            context="startup_discovery",
                        )
                        if pair_volumes else 0.0
                    )  # Skip volume filter if no data
                    ...
                    if pair_volumes and min_volume < base_min_volume:
                        self.logger().info(
                            f"🛟 Startup liquidity fallback active: min volume €{base_min_volume:,.0f} -> €{min_volume:,.0f}"
                        )
                    self.logger().info(
                        f"🎯 Selected {len(self.monitored_coins)} pairs (min €{min_volume:,.0f} volume, spread < 0.5%, not blacklisted)")  # noqa: E501
                    ...
                    self._last_coin_discovery = self._clock()
        ...
        if self.coin_discovery_refresh_interval > 0:  # 0 = disabled
            time_since_last_discovery = self._clock() - self._last_coin_discovery
        ...
        time_since_last_update = self._clock() - getattr(self, "_last_trend_update", 0.0)
        ...
        if time_since_last_update < min_update_interval:
            ...
            try:
                ...
                await self._ensure_regime_macro_trends(load_history=True, update_prices=True)
                self._last_trend_update = self._clock()
    ...
    async def _ensure_historical_data_loaded(self) -> None:
        ...
        if self._clock() - last_check < check_interval:
            return

        self._last_historical_check = self._clock()
    ...
    async def _update_monitored_coins_from_pool(self, pair_volumes: dict, pair_spreads: dict):
        ...
        try:
            ...
            max_coins = self.config.max_coins_to_monitor
            base_min_volume = self._base_min_volume_threshold()
            min_volume = self._resolve_adaptive_min_volume_threshold(
                pair_volumes=pair_volumes,
                target_count=max(1, int(max_coins)),
                context="pool_refresh",
            )
            spread_limit = 0.005  # 0.5%
            blacklist = set(getattr(self.config, 'blacklist', []) or [])
            ...
            if min_volume < base_min_volume:
                self.logger().info(
                    f"🛟 Pool refresh liquidity fallback active: min volume €{base_min_volume:,.0f} -> €{min_volume:,.0f}"
                )
            ...
            if added or removed:
                ...
                for coin in added:
                    self._pool_join_time[coin] = now
                    if not self._has_orderbook_subscription(coin):
                        self._subscribe_to_orderbook(coin)
                for coin in removed:
    ...
    def _rotate_underperforming_coins(self):
        ...
        min_volume = self._resolve_adaptive_min_volume_threshold(
            pair_volumes=self.pair_volumes,
            target_count=max(1, len(self.monitored_coins)),
            context="rotation_pool",
        )
        spread_limit = 0.005  # 0.5%
        ...
        for i in range(num_replacements):
            ...
            if not self._has_orderbook_subscription(new_coin):
                self._subscribe_to_orderbook(new_coin)
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        # T4-R4: Warmup gate — block all new entries until on_start() reconciliation is done
        if not self._warmup_complete:
            self.logger().info(
                f"⏳ Warmup in progress ({self._warmup_steps_done}/{self._warmup_steps_total})"
                " — trading blocked until reconciliation complete"
            )
            return actions
        ...
        # ===== ST-07: DEFINITIVE BEAR POLICY =====
        # Deep BEAR blocks new entries. Shallow BEAR is allowed only through
        # explicit BEAR-light policy, with reduced size and divergence checks.
        if self._last_detected_regime == "BEAR" and not (self.active_coin and self.active_executor_id):
            bear_decision = decide_bear_policy(
                regime=self._last_detected_regime,
                score=self._last_regime_score,
                regime_cfg=getattr(self.config, 'adaptive_regime_detection', {}) or {},
                adaptive_filters_cfg=getattr(self.config, 'adaptive_filters', None) or {},
            )

            if bear_decision.block_new_entries:
                score_str = f"{bear_decision.score:.1f}" if bear_decision.score is not None else "?"
                self.logger().info(
                    f"🐻 REGIME KILL: BEAR policy={bear_decision.policy} "
                    f"score={score_str} threshold={bear_decision.threshold:.1f} "
                    f"reason={bear_decision.reason}"
                )
                self._emit_regime_denial(
                    symbol="MARKET",
                    reason_code=ReasonCode.REGIME_BEAR_BLOCKED,
                    reason_msg=bear_decision.reason,
                    correlation_id=str(uuid.uuid4()),
                    metadata=bear_decision.metadata(),
                )
                return actions

            if bear_decision.in_bear_light:
                self.logger().info(
                    f"🐻✨ BEAR-LIGHT: score={bear_decision.score:.1f} > "
                    f"threshold={bear_decision.threshold:.1f} → allowing conditional grid "
                    f"with size multiplier {bear_decision.size_multiplier:.2f}"
                )
        ...
        try:
            ...
            if available_slots <= 0:
                ...
                # ST-02: Hard universe-quality prefilter before trend/GridScore ranking.
                excluded_coins_with_active = (
                    excluded_coins_with_active
                    | self._apply_universe_quality_gate(excluded_coins_with_active)
                )
                ...
                # RegimeCoinSelector is not wired into this hot path, so enforce the
                # most important CHOP rule here: avoid trendy coins in a grid regime.
                regime_for_selection = str(getattr(self, "_last_detected_regime", "") or "").upper()
                regime_selection_cfg = getattr(self.config, "regime_coin_selection", {}) or {}
                if regime_for_selection == "CHOP" and top_coins:
                    chop_cap = float(regime_selection_cfg.get("chop_max_trend_pct", 3.0))
                    capped_top_coins = []
                    rejected_by_chop_cap = []
                    for sym in top_coins:
                        trend = self.trend_calculator.get_trend(sym)
                        trend_pct = getattr(trend, "consensus_trend_pct", None) if trend else None
                        if trend_pct is None and trend:
                            trend_pct = getattr(trend, "trend_pct", 0.0)
                        try:
                            trend_pct = float(trend_pct or 0.0)
                        except (TypeError, ValueError):
                            trend_pct = 0.0
                        if abs(trend_pct) <= chop_cap:
                            capped_top_coins.append(sym)
                        else:
                            rejected_by_chop_cap.append(f"{sym}={trend_pct:+.2f}%")
                    if rejected_by_chop_cap:
                        self.logger().info(
                            f"🔎 CHOP trend cap removed {len(rejected_by_chop_cap)} candidate(s) "
                            f"> {chop_cap:.2f}%: {', '.join(rejected_by_chop_cap[:5])}"
                        )
                    top_coins = capped_top_coins
            ...
            self._log_momentum_candidates_detect_only(top_coins)

            # ===== META COIN RANKER: Reorder top_coins by historical performance =====
            # Coins with better recent win-rate / PnL bubble to the front.
            # Falls back to original order silently if insufficient history.
            if self.meta_ranker and self._trade_labels and top_coins:
                try:
                    _meta_scores = {r.coin: r.score for r in self.meta_ranker.rank(self._trade_labels)}
                    if _meta_scores:
                        top_coins = sorted(top_coins, key=lambda c: _meta_scores.get(c, 0.0), reverse=True)
                        self.logger().debug(
                            "🏆 MetaCoinRanker reordered: "
                            + ", ".join(f"{c}={_meta_scores.get(c, 0.0):.2f}" for c in top_coins)
                        )
                except Exception as _mce:
                    self.logger().warning(f"⚠️  MetaCoinRanker failed: {_mce}")
            ...
            # ===== ST-07: BEAR-LIGHT DIVERGENCE FILTER =====
            ...
            if (
                ...
                and bear_decision.in_bear_light
            ):
                quote = self.config.quote_asset
                ...
                btc_24h = (
                    float(getattr(btc_trend_obj, 'trend_1440m', 0.0) or 0.0)
                    if btc_trend_obj is not None else None
                )
                coin_24h = (
                    float(getattr(coin_trend_obj, 'trend_1440m', 0.0) or 0.0)
                    if coin_trend_obj is not None else None
                )
                if btc_24h is not None and coin_24h is not None:
                    if not bear_light_divergence_passes(True, coin_24h, btc_24h):
                        self.logger().info(
                ...
                if not self._check_multi_timeframe_buy_conditions(best_coin):
                    ...
                    if trend_obj and hasattr(trend_obj, 'trend_240m') and hasattr(trend_obj, 'trend_60m'):
                        ...
                        if hasattr(trend_obj, 'long_trend_warmup') and trend_obj.long_trend_warmup:
                            if trend_obj.trend_240m <= warmup_4h_threshold:
                                rejection_reason = f"4h trend ({
                                    trend_obj.trend_240m:+.2f}%) <= {warmup_4h_threshold:+.2f}% (warm-up requires > {warmup_4h_threshold:+.2f}%)"
                                reason_code = ReasonCode.MTF_INSUFFICIENT
                            elif trend_obj.trend_60m < warmup_1h_threshold:
                                # Fix #4: below warmup 1h threshold is insufficient, not crash
                                rejection_reason = f"1h trend ({
                                    trend_obj.trend_60m:+.2f}%) < {warmup_1h_threshold:+.2f}% (warm-up requires >= {warmup_1h_threshold:+.2f}%)"
                                reason_code = ReasonCode.MTF_INSUFFICIENT
                            else:
        ...
        if self.active_coin and self.active_executor_id and use_multi_timeframe:
            ...
            if exit_reason:
                ...
                if self._is_executor_actually_active():
                    try:
                        _emergency_reasons = {
                            "emergency_exit", "hard_stop_exit", "stop_loss",
                        }
                        stop_action = self._create_stop_action(
                            emergency=exit_reason in _emergency_reasons,
                        ...
                        if stop_action:
                            ...
                            if self.active_coin:
                                ...
                                if exit_reason in _emergency_reasons:
                                    cooldown_sec = getattr(
                                        self.config,
                                        'emergency_exit_cooldown_sec',
                                        1800,  # 30 min default
                                    )
                                    ...
                                    self.logger().critical(
                                        f"🚫 T1-K3: {self.active_coin} blacklisted "
                                        f"for {cooldown_sec // 60}m after {exit_reason}"
                                    )
        ...
        if not best_coin:
            ...
            # WHY-NO-TRADE: emit a gate_denied event with the actual blocker.
            # TREND_TOO_LOW is emitted only when best trend is truly below min trend.
            if hasattr(self, 'event_logger') and self.event_logger and self.event_logger.enabled:
                try:
                    _best_sym = "MARKET"
                    _best_trend: float = 0.0
                    _qualifying_coins: int = 0
                    if hasattr(self.trend_calculator, '_debug_info'):
                        _di = self.trend_calculator._debug_info
                        if _di.get('best'):
                            _best_sym, _best_trend = _di['best']
                        _qualifying_coins = int(_di.get('all_count', 0) or 0)

                    _min_trend = float(self.config.trend_min_change_pct)
                    _reason_msg = rejection_reason or "No qualifying coin after all gates"
                    _reason_upper = _reason_msg.upper()
                    _reason_code = ReasonCode.NO_QUALIFYING_COIN

                    if _qualifying_coins <= 0 and _best_trend < _min_trend:
                        _reason_code = ReasonCode.TREND_TOO_LOW
                        _reason_msg = (
                            f"Best coin {_best_sym}: trend {_best_trend:+.3f}% "
                            f"< min {self.config.trend_min_change_pct}%"
                        )
                    elif "SLOT" in _reason_upper:
                        _reason_code = ReasonCode.SLOT_FULL
                    elif "MTF" in _reason_upper or "24H TREND" in _reason_upper or "4H TREND" in _reason_upper:
                        _reason_code = ReasonCode.MTF_INSUFFICIENT
                    elif "BEAR" in _reason_upper:
                        _reason_code = ReasonCode.REGIME_BEAR_BLOCKED

                    _regime = getattr(self, '_last_detected_regime', None) or "UNKNOWN"
                    self.event_logger.emit_gate_denied(
                        correlation_id=str(uuid.uuid4()),
                        symbol=_best_sym,
                        stage=Stage.COIN_SELECTION,
                        reason_code=_reason_code,
                        reason_msg=_reason_msg,
                        metadata={
                            "best_trend_pct": round(_best_trend, 4),
                            "min_trend_pct": self.config.trend_min_change_pct,
                            "qualifying_coins": _qualifying_coins,
                            "available_slots": max(0, max_slots - len(self.active_coins)),
                            "active_grids": len(self.active_coins),
                            "rejection_reason": rejection_reason,
                        },
                        connector=self.config.connector_name,
                        regime=_regime,
                    ...
                    self.logger().debug(f"WHY-NO-TRADE: failed to emit no-selection event: {_e}")
        ...
        if edge_cfg.get('enabled', True):
            try:
                ...
                # Expected grid spread per level — use ATR-computed range if available,
                # otherwise fall back to static config (avoids phantom edge for low-ATR coins).
                ...
                # Try to get actual ATR-based range (same logic as SmartEntry / _compute_grid_range)
                actual_range_pct: float | None = None
                # Use same ATR multipliers as grid creation for accurate range estimate
                _atr_mult_down = float(getattr(self.config, 'atr_multiplier_down', 1.5))
                _atr_mult_up = float(getattr(self.config, 'atr_multiplier_up', 2.0))
                _atr_total_mult = _atr_mult_down + _atr_mult_up  # e.g. 3.5× ATR matches grid creation
                if hasattr(self, '_last_atr_range_computed'):
                    actual_range_pct = self._last_atr_range_computed.get(best_coin)
                if actual_range_pct is None and self.trend_calculator:
                    try:
                        _trend_eg = self.trend_calculator.get_trend(best_coin)
                        if _trend_eg and len(_trend_eg.candles) >= 14:
                            _highs = [c.high for c in _trend_eg.candles[-14:]]
                            _lows = [c.low for c in _trend_eg.candles[-14:]]
                            _tr = [max(h - l, abs(h - _trend_eg.candles[-14:][i - 1].close if i > 0 else h - l),
                                       abs(l - _trend_eg.candles[-14:][i - 1].close if i > 0 else 0))
                                   for i, (h, l) in enumerate(zip(_highs, _lows))]
                            _atr_raw = sum(_tr) / len(_tr)
                            _mid = _trend_eg.candles[-1].close
                            if _mid > 0:
                                # Match grid creation: ATR × (atr_mult_down + atr_mult_up)
                                actual_range_pct = float(_atr_raw / _mid * 100) * _atr_total_mult
                    except Exception:
                        pass

                total_range = actual_range_pct if actual_range_pct is not None else (range_down + range_up)
                # Divide by (num_grids - 1) gaps — grid creation spaces levels over n-1 intervals
                grid_spread_pct = total_range / max(1, num_grids - 1)
                # Apply same min_spacing floor as grid creation — without this ST-12 underestimates
                # actual spacing and blocks coins that would have profitable grids.
                _cfg_min_spacing = float(getattr(self.config, 'min_grid_level_spacing_pct', 0.60))
                grid_spread_pct = max(grid_spread_pct, _cfg_min_spacing)
                ...
                if expected_edge < min_edge - 1e-9:  # 1e-9% epsilon voor floating-point tolerantie
                    self.logger().info(
                    ...
                    if hasattr(self, 'event_logger') and self.event_logger and self.event_logger.enabled:
                        self.event_logger.emit_gate_denied(
                            correlation_id=str(uuid.uuid4()),
                            symbol=best_coin,
                            stage=Stage.EDGE_GATE,
                            reason_code=ReasonCode.EDGE_GATE_REJECTED,
                            reason_msg=f"edge {expected_edge:.3f}% < min {min_edge:.3f}%",
                            metadata={
                                "grid_spread_pct": round(grid_spread_pct, 4),
        ...
        if self._should_create_new_grid(best_coin):
            ...
            if not slots_available and self.active_coin and self.active_executor_id and self._is_executor_actually_active():
                ...
                if has_open_position:
                    ...
                    try:
                        stop_action = self._create_stop_action(stop_reason="switch_with_open_position")
                        if stop_action:
                            actions.append(stop_action)
                    ...
                    try:
                        stop_action = self._create_stop_action(stop_reason="switch_without_open_position")
                        if stop_action:
                            actions.append(stop_action)
                            if self.active_coin:
                                from multi_coin_grid_pro.core.exit_types import ExitType as _ExitType
            ...
            try:
                ...
                # ============================================================
                raw_available_balance, available_balance, budget_reserved = self._get_quote_balances_for_allocation()
                available_balance = self._apply_capital_reserve_to_balance(available_balance)

                # Get current regime for slot calculation
                current_regime = getattr(self, "_last_detected_regime", None) or "baseline"
                try:
                    if self.market_regime_filter:
                        regime_state = self.market_regime_filter.get_market_regime_state()
                        if regime_state:
                ...
                if not allocation.is_feasible:
                    if should_log("insufficient_capital", interval_sec=300):
                        self.logger().warning(
                            f"❌ Cannot create grid: {allocation.reason} "
                            f"(available: {available_balance:.2f} {self.config.quote_asset}) "
                ...
                # ===== ST-07: BEAR-LIGHT SIZE MULTIPLIER =====
                # Only conditional shallow-BEAR entries receive reduced size.
                bear_decision = decide_bear_policy(
                    regime=self._last_detected_regime,
                    score=self._last_regime_score,
                    regime_cfg=getattr(self.config, 'adaptive_regime_detection', {}) or {},
                ...
                if bear_decision.in_bear_light:
                    bear_size_mult = bear_decision.size_multiplier
                ...
                if self.max_simultaneous_coins > 1:
                    total_balance_for_cap = raw_available_balance
                ...
                admitted, adjusted_size = self._evaluate_entry_admission(
                    symbol=best_coin,
                ...
                budget_check = self.budget_allocator.check_budget(
                    total_balance=raw_available_balance,
                ...
                if grid_action:
                    ...
                    try:
                        self.execution_funnel.record_started(best_coin)
                    except Exception:
                        pass
        ...
        if len(top_coins) > 1 and len(self.active_coins) < max_slots_for_loop:
            ...
            for coin_index, candidate_coin in enumerate(top_coins[1:], start=2):  # Skip first coin (already processed)
                ...
                try:
                    ...
                    # ============================================================
                    raw_available_balance, available_balance, _budget_reserved = self._get_quote_balances_for_allocation()
                    available_balance = self._apply_capital_reserve_to_balance(
                        available_balance,
                        log_prefix="      ",
                    )
                    ...
                    loop_current_regime = (
                        current_regime if 'current_regime' in dir()
                        else (getattr(self, "_last_detected_regime", None) or "baseline")
                    )
                    allocation = self.dynamic_slot_manager.calculate_optimal_allocation(
                        available_balance=available_balance,
                        min_order_amount=Decimal(str(self.config.min_order_amount_quote)),
                        preferred_grids=self.config.num_grids,
                        max_coins=self.max_simultaneous_coins,
                        current_regime=loop_current_regime
                    )
                    ...
                    if grid_action:
                        actions.append(grid_action)
                        try:
                            self.execution_funnel.record_started(candidate_coin)
                        except Exception:
                            pass
                        applied_notional = Decimal(str(grid_action.executor_config.total_amount_quote))
    ...
    def _is_executor_actually_active(self) -> bool:
        ...
        if not active_executor:
            ...
            tracked_executor_id = self.active_executor_id
            tracked_coin = None
            if failed_executor and getattr(failed_executor, "config", None):
                tracked_coin = getattr(failed_executor.config, "trading_pair", None)
            if not tracked_coin:
                tracked_coin = next(
                    (coin for coin, executor_id in self.active_coins.items()
                     if executor_id == tracked_executor_id),
                    None,
                )
            if not tracked_coin:
                tracked_coin = self.active_coin

            # Reset exposure for the executor's coin if it exists. In multi-coin
            # mode active_coin may point at a different slot than active_executor_id.
            if tracked_coin and tracked_coin in self.current_exposure_per_coin:
                old_exposure = self.current_exposure_per_coin[tracked_coin]
                self.total_exposure -= old_exposure
                del self.current_exposure_per_coin[tracked_coin]
                self.logger().info(
                    f"📊 Reset exposure: {tracked_coin} (was €{old_exposure:.2f}), "
                    f"Total now €{self.total_exposure:.2f}"
                )
            # MULTI-COIN: Also remove from active_coins
            if tracked_coin and tracked_coin in self.active_coins:
                # US-004: Release budget reservation for this executor
                executor_id = self.active_coins[tracked_coin]
                self.budget_allocator.release(executor_id)
                del self.active_coins[tracked_coin]
                self.logger().info(f"📊 Removed {tracked_coin} from active_coins: {list(self.active_coins.keys())}")
            ...
            if tracked_coin and tracked_coin in self.price_history_for_volatility:
                del self.price_history_for_volatility[tracked_coin]
                self.logger().debug(f"🧹 Cleaned up price history for {tracked_coin}")

            self.active_executor_id = None
            if self.active_coin == tracked_coin:
                self.active_coin = None
            return False
    ...
    def _can_safely_close_position(self, trading_pair: str, executor_info: ExecutorInfo) -> tuple[bool, str]:
        ...
        try:
            connector = getattr(self, "connector", None)
            if connector is None:
                connector = getattr(self, "connectors", {}).get(self.config.connector_name)
            if connector is None:
                return False, "No connector available for pre-close validation"
            ...
            if hasattr(actual_balance, "__await__"):
                try:
                    actual_balance.close()
                except Exception:
                    pass
                return True, "Validation skipped: async balance unavailable"
            try:
                actual_balance = Decimal(str(actual_balance))
            except Exception:
                return True, "Validation skipped: exchange balance unavailable"
            ...
            try:
                ...
                min_order_size = trading_rule.min_order_size
                min_notional = getattr(trading_rule, "min_notional", None)
                if min_notional is None:
                    min_notional = getattr(trading_rule, "min_notional_size", Decimal("0"))
                try:
                    min_order_size = Decimal(str(min_order_size))
                    min_notional = Decimal(str(min_notional))
                except Exception:
                    return True, "Validation skipped: trading rule limits unavailable"
            except Exception as e:
            ...
            if actual_balance < min_order_size:
                mid_for_value = connector.get_mid_price(trading_pair) if hasattr(connector, "get_mid_price") else None
                value_eur = actual_balance * mid_for_value if mid_for_value else Decimal("0")
                return False, f"Dust: {actual_balance:.8f} {base_asset} < min {min_order_size:.8f} (€{value_eur:.2f})"

            # Check 2: Is value > min notional?
            mid_price = connector.get_mid_price(trading_pair) if hasattr(connector, "get_mid_price") else None
    ...
    def _clear_position_tracking_after_confirmed_close(
        self,
        trading_pair: Optional[str],
        executor_id: Optional[str],
        reason: str = "confirmed_close",
    ) -> None:
        """Clear entry/exposure only after the executor is confirmed closed."""
        if not trading_pair:
            return

        if trading_pair in self.entry_prices:
            entry_price = self.entry_prices[trading_pair]
            del self.entry_prices[trading_pair]
            entry_price_store = getattr(self, "entry_price_store", None)
            if entry_price_store:
                try:
                    entry_price_store.delete(trading_pair)
                except Exception:
                    pass
            self.logger().info(
                f"📌 Cleared entry price tracking for {trading_pair} after confirmed close "
                f"(was €{entry_price:.4f}, reason={reason})"
            )

        current_exposure_per_coin = getattr(self, "current_exposure_per_coin", {})
        if trading_pair in current_exposure_per_coin:
            exposure = current_exposure_per_coin[trading_pair]
            self._update_exposure_tracking(trading_pair, -exposure)
            self.logger().info(
                f"📊 Cleared exposure tracking for {trading_pair} after confirmed close "
                f"(was €{exposure:.2f}, reason={reason})"
            )

        if trading_pair in self.active_coins:
            tracked_executor_id = self.active_coins[trading_pair]
            if executor_id is None or tracked_executor_id == executor_id:
                if hasattr(self, 'budget_allocator'):
                    self.budget_allocator.release(tracked_executor_id)
                del self.active_coins[trading_pair]
                self.logger().info(
                    f"📊 Removed {trading_pair} from active_coins after confirmed close: "
                    f"{list(self.active_coins.keys())}"
                )

        try:
            bucket_tracker = getattr(self, "bucket_tracker", None)
            if bucket_tracker:
                bucket_tracker.clear_coin(trading_pair)
        except Exception as _bte:
            self.logger().warning(f"⚠️  Bucket tracker clear_coin failed: {_bte}")

        price_history = getattr(self, "price_history_for_volatility", {})
        if trading_pair in price_history:
            del price_history[trading_pair]
            self.logger().debug(f"🧹 Cleaned up price history for {trading_pair}")

        active_coin = getattr(self, "active_coin", None)
        active_executor_id = getattr(self, "active_executor_id", None)
        if active_coin == trading_pair and (executor_id is None or active_executor_id == executor_id):
            self.active_coin = None
            self.active_executor_id = None
            self._active_executor_notional = Decimal("0")

    def _fee_aware_non_emergency_exit_allowed(
        self,
        trading_pair: str,
        executor_info: Optional[ExecutorInfo],
        current_price: Decimal,
        proposed_close_reason: str,
    ) -> bool:
        """
        Prevent trend/max-hold rotation exits from selling below fee-aware
        break-even. Emergency exits deliberately bypass this guard.
        """
        emergency_reasons = {"emergency_exit", "hard_stop_exit", "stop_loss"}
        fee_cfg = getattr(self.config, 'fee_aware_filter', {}) or {}
        taker_fee_pct = Decimal(str(fee_cfg.get('taker_fee_pct', 0.35)))
        expected_exit_fee_rate = taker_fee_pct / Decimal("100")

        if proposed_close_reason in emergency_reasons:
            self.logger().warning(
                f"🧮 FEE_AWARE_EXIT_CHECK controller: avg_entry=0, break_even=0, "
                f"expected_exit_fee=0, current_price={current_price:.10f}, "
                f"proposed_close_reason={proposed_close_reason}, allowed=True emergency_bypass=True"
            )
            return True

        if executor_info is None or not executor_info.custom_info:
            self.logger().warning(
                f"🧮 FEE_AWARE_EXIT_CHECK controller: missing executor state for {trading_pair}; "
                f"proposed_close_reason={proposed_close_reason}, allowed=True"
            )
            return True
        ...
        avg_entry = Decimal(str(
            ...
            or custom_info.get("entry_price")
            or self.entry_prices.get(trading_pair, Decimal("0"))
        ))
        position_size_quote = Decimal(str(custom_info.get("position_size_quote", Decimal("0"))))
        position_fees_quote = Decimal(str(custom_info.get("position_fees_quote", Decimal("0"))))

        if avg_entry <= Decimal("0") or position_size_quote <= Decimal("0"):
            self.logger().info(
                f"🧮 FEE_AWARE_EXIT_CHECK controller: avg_entry={avg_entry:.10f}, "
                f"break_even=0, expected_exit_fee=0, current_price={current_price:.10f}, "
                f"proposed_close_reason={proposed_close_reason}, allowed=True no_open_position=True"
            )
            return True
        ...
        decision = fee_aware_exit_decision(
            ...
            buy_fees_quote=position_fees_quote,
            current_price=current_price,
            expected_exit_fee_rate=expected_exit_fee_rate,
        )

        log_payload = (
            f"avg_entry={decision.avg_entry:.10f}, "
            f"break_even={decision.break_even:.10f}, "
            f"expected_exit_fee={decision.expected_exit_fee:.8f}, "
            f"current_price={current_price:.10f}, "
            f"proposed_close_reason={proposed_close_reason}, "
            f"allowed={decision.allowed}"
        )
        if decision.allowed:
            self.logger().info(f"🧮 FEE_AWARE_EXIT_CHECK controller: {log_payload}")
        else:
            self.logger().warning(f"🛡️ FEE_AWARE_EXIT_BLOCKED controller: {log_payload}")
                    ...
                    if executor.close_type and 'STOP_LOSS' in str(executor.close_type):
                        if self.active_coin not in self.stop_loss_triggered:
                            ...
                            if self.active_coin in self.entry_prices:
                                ...
                                # T1-K4: Remove from SQLite
                                if self.entry_price_store:
                                    try:
                                        self.entry_price_store.delete(self.active_coin)
                                    except Exception:
                                        pass

                            # T1-K3: Blacklist coin after stop-loss
                            self._add_to_blacklist(
                                self.active_coin,
                                "T1-K3:stop_loss",
                                current_time,
    ...
    def _add_to_blacklist(self, symbol: str, reason: str, now: float,
                          duration_override: Optional[float] = None) -> None:
        """
        ...
        Args:
            ...
            now: Current timestamp
            duration_override: If set, use this duration instead of config default
        """
        # Get blacklist duration from config (default 30 min = 1800 sec)
        if duration_override is not None:
            blacklist_duration = duration_override
        else:
    ...
    # ==========================================================================
    # T1-K2: KILL SWITCH — STOP ALL ACTIVE EXECUTORS
    # ==========================================================================

    async def _kill_switch_stop_all_executors(self) -> None:
        """
        Emergency stop: issue StopExecutorAction for every active executor.

        Called when RiskGuardV2.check_limits() returns False or errors.
        Without this, the old code only skipped the current tick — active
        executors (open grids, open orders) kept running indefinitely.
        """
        # Only send stop actions once per kill-switch activation
        if self._kill_switch_actions_sent:
            return

        controller_id = getattr(self.config, 'id', None) or getattr(
            self.config, 'controller_name', 'multi_coin_grid'
        )
            ...
            if executor.is_active:
                try:
                    stop_action = StopExecutorAction(
                        ...
                        keep_position=False,  # Close positions, don't leave them open
                    )
                    self.executors_update_event.set()
                    # Use the executor handler to process the stop action
                    if hasattr(self, 'executor_orchestrator'):
                        self.executor_orchestrator.execute_action(stop_action)
                    stop_count += 1
                    tp = getattr(
                        getattr(executor, "config", None), "trading_pair", "?"
                    )
                    self.logger().critical(
                        f"🛑 T1-K2: Stopping executor {executor.id[:8]}... "
                        f"({tp}) — kill switch"
                    )
                except Exception as e:
                    self.logger().error(
                        f"T1-K2: Failed to stop executor {executor.id[:8]}...: {e}"
                    )

        self._kill_switch_actions_sent = True

        summary = self.pnl_tracker_v2.get_summary()
        msg = (
            f"🚨 KILL SWITCH: Stopped {stop_count} active executors.\n"
            f"   Daily PnL: {summary['daily_pnl_pct']:.2f}%\n"
            f"   Realized: €{summary['realized_pnl']:+.2f}\n"
            f"   Equity: €{summary['equity']:.2f}"
        )
        self.logger().critical(msg)
        if hasattr(self, 'telegram_alerter'):
            self.telegram_alerter.critical(msg)
    ...
    def _sync_risk_state(self) -> None:
        ...
        for executor in self.executors_info:
            status_name = getattr(getattr(executor, "status", None), "name", str(getattr(executor, "status", "")))
            if position_tracking_cleanup_allowed(status_name, executor.is_active):
                trading_pair = getattr(getattr(executor, "config", None), "trading_pair", None)
                self._clear_position_tracking_after_confirmed_close(
                    trading_pair=trading_pair,
                    executor_id=executor.id,
                    reason="executor_terminated",
                )
        ...
        # Budget reservations are pessimistic and live only in memory. Keep them
        # reconciled with real active executors so stale closed grids cannot
        # starve the next entry cycle.
        if hasattr(self, "budget_allocator"):
            try:
                active_executor_ids = {
                    executor.id
                ...
                grace_times = getattr(self, "_executor_creation_times", {}) or {}
                for executor_id, creation_time in grace_times.items():
                    if now - creation_time < 30:
                        active_executor_ids.add(executor_id)
                cleaned = self.budget_allocator.sync_with_active_executors(active_executor_ids, now)
                if cleaned:
                    self.logger().info(
                        f"🧹 US-004 budget sync cleaned {cleaned} orphaned reservation(s)"
                    )
            except Exception as exc:
                self.logger().warning(f"⚠️  US-004 budget sync failed: {exc}")
        ...
        for executor in self.executors_info:
            if executor.status == RunnableStatus.TERMINATED:
                if executor.id not in self._realised_executors_tracked:
                    ...
                    # ==============================================================
                    # ORPHAN RUNTIME TRIGGER: When an executor closes as FAILED
                    # (especially INSUFFICIENT_BALANCE), the buy may have filled
                    # but the sell was never placed. Schedule an orphan check for
                    # this specific pair so the position gets auto-sold immediately
                    # rather than waiting until the next bot restart.
                    # ==============================================================
                    try:
                        from hummingbot.strategy_v2.models.executors import CloseType as _CT
                        if executor.close_type == _CT.FAILED:
                            ci = getattr(executor, "custom_info", {}) or {}
                            early_stop = ci.get("early_stop_reason", "")
                            buy_q = Decimal(str(ci.get("realized_buy_size_quote", 0)))
                            sell_q = Decimal(str(ci.get("realized_sell_size_quote", 0)))
                            position_q = Decimal(str(ci.get("position_size_quote", 0)))
                            tracked_base = self._tracked_orphan_base_from_custom_info(ci)
                            if early_stop == "INSUFFICIENT_BALANCE" or tracked_base > Decimal("0") or buy_q > 0:
                                self.logger().warning(
                                    f"🔍 ORPHAN-TRIGGER: {trading_pair} executor FAILED "
                                    f"(early_stop={early_stop}, buy={buy_q:.2f}, sell={sell_q:.2f}, "
                                    f"position={position_q:.2f}, tracked_base={tracked_base:.8f}) "
                                    f"— scheduling orphan check"
                                )
                                import asyncio
                                asyncio.ensure_future(
                                    self._check_single_orphan(
                                        trading_pair,
                                        max_sell_amount=tracked_base if tracked_base > Decimal("0") else None,
                    ...
                    # net_pnl_quote = -(buy_total + fees) — a phantom -100% loss.
                    # The coins WERE sold (balance=0) but the fill wasn't recorded.
                    # Correct: only count recorded fees as the loss.
                    # ==============================================================
                    try:
                        from hummingbot.strategy_v2.models.executors import CloseType as _CT
                        if executor.close_type == _CT.FAILED:
                            ...
                            if exec_buy > 0 and exec_sell < exec_buy:
                                ...
                                self.logger().warning(
                                    f"⚠️ PHANTOM_LOSS_FIX: {trading_pair} FAILED "
                                    f"buy={exec_buy:.2f} sell={exec_sell:.2f} "
                        ...
                        try:
                            ...
                            # ==============================================================
                            # ITEM 1: Register close with GlobalRiskManager
                            # Feeds: per-coin daily kill switch (-1R/-2R), exit-type cooldown,
                            # consecutive-loss tracking, and daily loss accumulation.
                            # Without this call, is_coin_disabled_today() / is_coin_halved_today()
                            # always return False, making the per-coin kill switch dead.
                            # ==============================================================
                            try:
                                self.risk_manager.register_close_trade(
                                    ...
                                    now=now,
                                    exit_type=_determined_exit_type,
                                )
                                self.logger().debug(
                                    f"🔄 risk_manager.register_close_trade: {trading_pair} "
                                    f"pnl={realised_pnl:+.4f} exit={_determined_exit_type}"
                                )
                            except Exception as _rct_e:
                                self.logger().error(
                                    f"register_close_trade failed for {trading_pair}: {_rct_e}"
                                )
                        except Exception as e:
                        ...
                        try:
                            _now_ts = int(self._clock())
                            _ctx = self._entry_context_by_symbol.pop(trading_pair, {})
                            _close_ts = int(getattr(audit, "end_ts", _now_ts) or _now_ts)
                            _open_ts = int(_ctx.get("timestamp_open") or getattr(audit, "start_ts", _close_ts - 3600))
                            _session = _ctx.get("session") or session_from_utc(float(_open_ts))
                            _lbl = TradeLabel(
                                coin=trading_pair,
                                timestamp_open=_open_ts,
                                timestamp_close=_close_ts,
                                session=_session,
                                regime=_ctx.get("regime", self._last_detected_regime or ""),
                                volatility_state="normal",
                                spread_at_entry=float(_ctx.get("spread_at_entry", self.pair_spreads.get(trading_pair, 0.0))),
                                depth_at_entry=float(_ctx.get("depth_at_entry", 0.0)),
                                quality_score=int(_ctx.get("quality_score", self._quality_score)),
                                entry_reason="grid",
                                ...
                                mfe_pct=0.0,
                                mae_pct=0.0,
                                reentry=self._is_reentry_same_session(trading_pair, _session),
                                cycle_number=self._entry_cycle_number(trading_pair, _close_ts),
                                connector=self.config.connector_name,
                            )
                            ...
                            if self.trade_label_store:
                                self.trade_label_store.record(_lbl)
                        except Exception as _e:
                        ...
                        # ==============================================================================
                        # Without this, pnl_tracker_v2.daily_pnl_pct() always returns 0%,
                        # making RiskGuardV2.check_limits() unable to ever trigger.
                        try:
                            from multi_coin_grid_pro.core.models import TradeFill as PnlTradeFill
                            executor_config = getattr(executor, "config", None)
                            position_size = Decimal(str(
                                getattr(executor_config, "total_amount_quote", 50.0)
                            ))
                            entry_price_val = self.entry_prices.get(
                                trading_pair,
                                getattr(executor_config, "start_price", Decimal("0"))
                            )
                            if entry_price_val and entry_price_val > 0 and position_size > 0:
                                qty = position_size / entry_price_val
                                # Synthesize a buy fill (entry)
                                buy_fill = PnlTradeFill(
                                    symbol=trading_pair,
                                    side="buy",
                                ...
                                # Synthesize a sell fill (exit)
                                exit_price_val = entry_price_val
                                if position_size > 0:
                                    pnl_ratio = realised_pnl / position_size
                                    exit_price_val = entry_price_val * (Decimal("1") + pnl_ratio)
                                sell_fill = PnlTradeFill(
                                    symbol=trading_pair,
                                    side="sell",
                                ...
                                self.logger().info(
                                    f"💰 T1-K1: PnL tracker updated for {trading_pair}: "
                                    f"realized={realised_pnl:+.4f}, "
                                    f"daily_pnl={self.pnl_tracker_v2.daily_pnl_pct():.2f}%"
                                )
                        except Exception as e:
                            self.logger().error(f"T1-K1: Failed to update pnl_tracker_v2: {e}")
                        # ==============================================================================
                        ...
                        if executor.id not in self._processed_timeout_executors:
                            ...
                            # T1-K3: Also blacklist on STOP_LOSS (emergency exit cooldown)
                            emergency_close_types = {
                                CloseType.STOP_LOSS,
                            }

                            if executor.close_type in timeout_close_types:
                                # Timeout close - add to blacklist with close_cooldowns duration
                                reason = executor.close_type.name if executor.close_type else "TIMEOUT"
                                cooldown_sec = self._get_close_cooldown_sec(executor.close_type, trading_pair)
                                self._add_to_blacklist(trading_pair, reason, now, duration_override=cooldown_sec)
                            elif executor.close_type in emergency_close_types:
                                # T1-K3: Stop-loss close — blacklist to prevent re-entry
                                cooldown_sec = self._get_close_cooldown_sec(executor.close_type, trading_pair)
                                self._add_to_blacklist(
                                    trading_pair,
                                    f"T1-K3:{executor.close_type.name}",
                                    now,
                                    duration_override=cooldown_sec,
                                )
                            else:
                                # T2-O8: Short rotation cooldown after normal/TAKE_PROFIT close
                                # Prevents immediate re-selection of the same coin
                                rotation_cooldown = getattr(
                                    self.config, 'rotation_cooldown_after_close_sec', 300
                                )
                                if rotation_cooldown > 0:
                                    close_type_name = (
                                        executor.close_type.name
                                        if executor.close_type
                                        else "UNKNOWN"
                                    )
                                    self._add_to_blacklist(
                                        trading_pair,
                                        f"T2-O8:ROTATION_COOLDOWN:{close_type_name}",
                                        now,
                                        duration_override=rotation_cooldown,
                                    )
                                else:
                                    close_type_name = (
                                        executor.close_type.name
                    ...
                    try:
                        _exec_fees = Decimal(str(executor.cum_fees_quote or 0))
                        _gross_pnl = realised_pnl + _exec_fees  # undo fee deduction to get gross
                        _close_name = str(executor.close_type) if executor.close_type else "?"
                        self.logger().info(
                            f"📊 ROUNDTRIP {trading_pair}: "
                            f"gross={float(_gross_pnl):+.4f} fees={float(_exec_fees):.4f} "
                            f"net={float(realised_pnl):+.4f} | "
                            f"fee_bps={float(_exec_fees / max(_exec_fees + abs(realised_pnl), Decimal('0.01')) * 10000):.0f}bps "
                            f"close={_close_name}"
                        )
                        if realised_pnl < 0:
                            self._loss_streaks[trading_pair] = self._loss_streaks.get(trading_pair, 0) + 1
                            streak = self._loss_streaks[trading_pair]
                            cc = getattr(self.config, 'close_cooldowns', {}) or {}
                            max_streak = int(cc.get('max_streak_before_blacklist', 3))
                            self.logger().warning(
                                f"⚠️  LOSS_STREAK {trading_pair}: {streak}/{max_streak} "
                                f"consecutive losses (net={float(realised_pnl):+.4f})"
                            )
                        else:
                            if trading_pair in self._loss_streaks:
                                self.logger().info(
                                    f"✅ STREAK_RESET {trading_pair}: "
                                    f"streak was {self._loss_streaks[trading_pair]}, reset to 0"
                                )
                            self._loss_streaks[trading_pair] = 0
                    except Exception as _streak_e:
                        self.logger().debug(f"Loss streak tracking error: {_streak_e}")
        ...
        if self.active_coin and self.active_executor_id:
            ...
            if executor_info:
                ...
                # T1-K1: Update unrealized PnL in pnl_tracker_v2 for kill switch
                try:
                    prices: Dict[str, Decimal] = {}
                    for ex in self.executors_info:
                        if ex.is_active:
                            tp = getattr(getattr(ex, "config", None), "trading_pair", None)
                            sp = getattr(getattr(ex, "config", None), "start_price", None)
                            if tp and sp:
                                # Estimate current price from net_pnl + entry
                                entry = Decimal(str(sp))
                                total_q = Decimal(str(
                                    getattr(ex.config, "total_amount_quote", 50)
                                ))
                                if entry > 0 and total_q > 0:
                                    pnl_q = Decimal(str(ex.net_pnl_quote))
                                    pnl_ratio = pnl_q / total_q
                                    prices[tp] = entry * (Decimal("1") + pnl_ratio)
                    if prices:
                        self.pnl_tracker_v2.update_unrealized(prices)
                except Exception:
                    pass  # Non-critical: unrealized update is best-effort
    ...
    def _check_professional_exit_signals(self) -> List:
        ...
        for trading_pair, executor_id in list(self.active_coins.items()):
            try:
                # Find the executor. If it exists but is no longer active, this is a normal
                # close/store lifecycle and should not be treated as an exchange orphan warning.
                executor_any = next(
                    (e for e in self.executors_info if e.id == executor_id),
                    None
                )
                executor = executor_any if executor_any and executor_any.is_active else None
                if not executor:
                    stale_key = f"{trading_pair}_{executor_id}"
                    if executor_any and not executor_any.is_active:
                        self.logger().info(
                            f"🧹 Cleaning closed executor reference: {trading_pair} "
                            f"(executor {executor_id[:8]}... is no longer active)"
                        )
                        self._clear_position_tracking_after_confirmed_close(
                            trading_pair=trading_pair,
                            executor_id=executor_id,
                            reason="executor_inactive",
                        )
                    ...
                    if self._stale_executor_counts[stale_key] >= 10:
                        self.logger().warning(
                            f"🧹 Cleaning up stale local executor reference: {trading_pair} "
                        ...
                        self._clear_position_tracking_after_confirmed_close(
                            trading_pair=trading_pair,
                            executor_id=executor_id,
                            reason="missing_executor_reference",
                        )
                        if executor_id in self._executor_creation_timestamps:
                            del self._executor_creation_timestamps[executor_id]
                        ...
                        # Telegram is opt-in here: a missing local reference is not proof of
                        # an exchange orphan and the old warning created noisy false alarms.
                        if (
                            getattr(self.config, "send_stale_executor_telegram_alerts", False)
                            and hasattr(self, 'telegram_alerter')
                            and self.telegram_alerter.enabled
                        ):
                            self.telegram_alerter.warning(
                                f"<b>🧹 STALE EXECUTOR CLEANED</b>\n\n"
                                f"• Pair: <b>{trading_pair}</b>\n"
                                f"• Executor: {executor_id[:16]}...\n"
                                f"• Reason: local executor reference disappeared from orchestrator\n\n"
                                f"Check exchange only if balances or open orders look unexpected."
                            )
                    continue
                ...
                # PROFIT_LOCK grace period: don't fire within the first N seconds after
                # executor creation. Bug: market spike within seconds of opening sets a high
                # water mark, then normal pullback immediately triggers PROFIT_LOCK and the
                # executor enters a multi-hour stuck close cycle.
                profit_lock_grace_sec = float(
                    getattr(self.config, 'profit_lock_grace_period_sec', 600)  # 10 min default
                )
                time_held_sec = now - self._executor_creation_timestamps.get(executor_id, now)
                if action == "PROFIT_LOCK" and time_held_sec < profit_lock_grace_sec:
                    self.logger().debug(
                        f"⏸️  US-005 PROFIT_LOCK suppressed for {trading_pair}: "
                        f"executor too young ({time_held_sec / 60:.1f} min < {profit_lock_grace_sec / 60:.0f} min grace)"
                    )
                    action = "HOLD"
                ...
                if action in ("STOP_LOSS", "TIME_STOP", "PROFIT_LOCK"):
                    ...
                    # Create stop action — pass the reason so fee-aware timeout bypass works
                    from hummingbot.strategy_v2.models.executor_actions import StopExecutorAction
                    controller_id = self.config.id or getattr(
                        self.config, 'controller_name', 'multi_coin_grid'
                    )
                    stop_action = StopExecutorAction(
                        controller_id=controller_id,
                        executor_id=executor_id,
                        early_stop_reason=self._early_stop_reason_from_stop_reason(action.lower()),
                    )
                    actions.append(stop_action)
                    ...
                    if self.event_logger:
                        from multi_coin_grid_pro.core.reason_codes import ReasonCode, Stage
                        self.event_logger.emit_gate_denied(
                            correlation_id=f"exit_{executor_id}_{int(self._clock())}",
                            symbol=trading_pair,
    ...
    async def _run_dynamic_pair_scan(self) -> None:
        ...
        try:
            ...
            if scanned_count > 0:
                ...
                subscribed_count = 0
                pairs_to_ensure = set(self.monitored_coins) | active_pairs | set(self.active_coins.keys())
                for pair in sorted(pairs_to_ensure):
                    if not self._has_orderbook_subscription(pair):
                        self._subscribe_to_orderbook(pair)
                        subscribed_count += 1
                if subscribed_count:
                    self.logger().info(
                        f"📚 US-007: Subscribed {subscribed_count} monitored pair(s) to orderbooks"
                    )
    ...
    def _should_create_new_grid(self, best_coin: str) -> bool:
        ...
        if max_slots_for_capital > 1:
            ...
            if current_count < max_slots_for_capital:
                # ── Budget pre-check: avoid grid restart spam ──
                try:
                    # Use balance_currency if set (e.g. OKX EU: quote_asset=USD but wallet holds USDC)
                    _balance_ccy = getattr(self.config, "balance_currency", None) or self.config.quote_asset
                    avail = self.connector.get_available_balance(_balance_ccy)
                except Exception:
                    avail = Decimal(str(self.config.total_amount_quote))

                effective = max(Decimal("0"), avail - self.budget_allocator.total_reserved)
                effective = self._apply_capital_reserve_to_balance(effective)
                grid_count_for_budget = int(getattr(self, "_dynamic_num_grids", None) or self.config.num_grids)
                try:
                    trend = self.trend_calculator.get_trend(best_coin)
                    if trend:
                        grid_count_for_budget = self._calculate_volatility_based_grid_count(best_coin, trend)
                except Exception:
                    grid_count_for_budget = int(getattr(self, "_dynamic_num_grids", None) or self.config.num_grids)
                min_grid_capital = (
                    Decimal(str(max(1, grid_count_for_budget)))
                    * Decimal(str(self.config.min_order_amount_quote))
                )
                if effective < min_grid_capital:
                    if should_log("budget_precheck_blocked", interval_sec=300):
                        self.logger().info(
                            f"💰 Budget pre-check: insufficient capital for {best_coin} "
                            f"(effective={effective:.2f}, need={min_grid_capital:.2f})"
                        )
                    return False
        ...
        if not self.active_coin and len(self.active_coins) == 0:
            ...
            time_since_start = self._clock() - self.bot_start_time
        ...
        if is_opportunity_switch and time_since_switch < min_hold_time:
            ...
            # AI-F1: Log rejected rotation (hold time)
            self._log_rotation_decision(
                symbol=best_coin, accepted=False,
                from_coin=self.active_coin,
                rejected_by="min_hold_time",
                reason_msg=f"Hold time {time_since_switch / 60:.1f}/{min_hold_time / 60:.1f} min",
                hold_time_sec=time_since_switch,
                from_trend_pct=float(active_trend_value),
                to_trend_pct=float(best_trend_value),
            )
            return False
        ...
        # AI-F1: Log accepted rotation decision
        self._log_rotation_decision(
            symbol=best_coin, accepted=True,
            from_coin=self.active_coin,
            hold_time_sec=time_since_switch,
    ...
    def _check_smart_entry_filter(self, symbol: str) -> bool:
        ...
        if self.pair_health_monitor and self.pair_health_monitor.is_quarantined(symbol):
            ...
            remaining = (info.release_at - self._clock()) / 60 if info else 0
            self.logger().debug(
            ...
            if not freshness.is_fresh:
                ...
                if self.event_logger:
                    try:
                        ...
                        self.event_logger.emit_gate_denied(
                            correlation_id=f"staleness_{symbol}_{self._clock()}",
                            symbol=symbol,
    ...
    def _regime_macro_pairs(self) -> List[str]:
        """Return macro pairs used for regime/momentum context, outside grid selection."""
        quote = getattr(self.config, "quote_asset", None)
        if not quote:
            return []
        return [f"BTC-{quote}", f"SOL-{quote}"]

    def _macro_pairs_excluded_from_grid(self) -> set:
        monitored = set(getattr(self, "monitored_coins", []) or [])
        return {pair for pair in self._regime_macro_pairs() if pair not in monitored}

    def _get_macro_trend(self, pair: str):
        trend_map = getattr(self.trend_calculator, "trends", {}) if self.trend_calculator else {}
        trend = trend_map.get(pair)
        if trend is not None:
            return trend
        return getattr(self, "_regime_macro_trends", {}).get(pair)

    def _cache_regime_macro_trends(self, remove_from_grid_trends: bool = True) -> None:
        if not self.trend_calculator:
            return

        trend_map = getattr(self.trend_calculator, "trends", {})
        cache = getattr(self, "_regime_macro_trends", {})
        excluded = self._macro_pairs_excluded_from_grid()
        for pair in self._regime_macro_pairs():
            trend = trend_map.get(pair)
            if trend is not None:
                cache[pair] = trend
                if remove_from_grid_trends and pair in excluded:
                    trend_map.pop(pair, None)
        self._regime_macro_trends = cache

    async def _ensure_regime_macro_trends(self, load_history: bool = False, update_prices: bool = True) -> None:
        """
        Keep BTC/SOL macro trends available without adding them to monitored_coins.

        TrendCalculator ranks every symbol in its trends map, so macro trends
        are temporarily inserted for loading/updating and then cached outside
        that map to avoid changing grid candidate selection.
        """
        if not self.trend_calculator:
            return

        pairs = self._regime_macro_pairs()
        if not pairs:
            return

        trend_map = getattr(self.trend_calculator, "trends", {})
        for pair, trend in getattr(self, "_regime_macro_trends", {}).items():
            if pair in pairs and pair not in trend_map:
                trend_map[pair] = trend

        previous_debug_info = getattr(self.trend_calculator, "_debug_info", None)
        previous_debug_copy = dict(previous_debug_info) if isinstance(previous_debug_info, dict) else previous_debug_info
        ...
        try:
            if load_history:
                ...
                for pair in pairs:
                    trend = trend_map.get(pair)
                    candles = getattr(trend, "candles", None) if trend else None
                    if not candles or len(candles) < 14:
                        missing_history.append(pair)
                if missing_history:
                    await self.trend_calculator.load_historical_data(missing_history)
            ...
            if update_prices:
                ...
                # Fallback: for macro pairs where batch price fetch fails (e.g.,
                # BTC-USD on Kraken due to XBT symbol mapping → no_price_in_batch),
                # try get_mid_price() which uses the cached order book (always fresh).
                connector = getattr(self.trend_calculator, "connector", None)
                if connector:
                    trend_map_now = getattr(self.trend_calculator, "trends", {})
                    now = self._clock()
                    for pair in pairs:
                        trend = trend_map_now.get(pair)
                        is_stale = trend is None or (now - getattr(trend, "last_updated", 0)) > 60
                        if is_stale:
                            try:
                                mid = connector.get_mid_price(pair)
                                if mid and float(mid) > 0:
                                    await self.trend_calculator._update_coin_trend_with_price(
                                        pair, float(mid)
                                    )
                                    self.logger().debug(
                                        f"🌡️ Macro trend mid_price fallback: {pair}={float(mid):.2f}"
                                    )
                            except Exception:
                                pass

            self._cache_regime_macro_trends(remove_from_grid_trends=True)
        except Exception as e:
            self.logger().warning(f"⚠️ Regime macro trend refresh skipped: {e}")
            self._cache_regime_macro_trends(remove_from_grid_trends=True)
        finally:
            if previous_debug_copy is not None:
                self.trend_calculator._debug_info = previous_debug_copy

    def _log_momentum_candidates_detect_only(self, top_candidates: List[str]) -> None:
        """
        Score and log momentum candidates without opening positions or orders.

        Phase 2 is detect-only: any error here skips momentum logging and the
        normal grid cycle continues untouched.
        """
        scorer = getattr(self, "momentum_candidate_scorer", None)
        if not scorer or not self.trend_calculator:
            return

        try:
            momentum_cfg = getattr(self.config, "momentum_sleeve", {}) or {}
            if self._momentum_config_mode(momentum_cfg) != "detect_only":
                return

            symbols = self._momentum_candidate_symbols(top_candidates)
            if not symbols:
                return

            quote = self.config.quote_asset
            btc_trend_obj = self._get_macro_trend(f"BTC-{quote}")
            regime = getattr(self, "_last_detected_regime", None) or "CHOP"
            ...
            for symbol in symbols:
                ...
                spread_pct = self._momentum_spread_pct(symbol)
                wick_risk = self._wick_ratio_to_momentum_risk(indicators.wick_ratio)
                candidate = scorer.score(
                    symbol=symbol,
                    trend_obj=trend,
                    btc_trend_obj=btc_trend_obj,
                    spread=spread_pct,
                    rsi=indicators.rsi_14,
                    wick_risk=wick_risk,
                    regime=regime,
                )
                self._log_momentum_candidate(candidate)
        except Exception as e:
            self.logger().warning(f"⚠️ Momentum detect-only scoring skipped: {e}")

    def _momentum_candidate_symbols(self, top_candidates: List[str]) -> List[str]:
        """Return up to 10 unique trend-ranked symbols for detect-only scoring."""
        symbols: List[str] = []
        excluded_macro_pairs = self._macro_pairs_excluded_from_grid()

        def add_symbol(value):
            if value and value not in excluded_macro_pairs and value not in symbols:
                symbols.append(value)

        if hasattr(self.trend_calculator, "_debug_info"):
            for item in self.trend_calculator._debug_info.get("top_10", [])[:10]:
                symbol = item[0] if isinstance(item, (tuple, list)) and item else item
                add_symbol(symbol)

        for symbol in top_candidates or []:
            add_symbol(symbol)

        return symbols[:10]

    def _log_momentum_candidate(self, candidate) -> None:
        if candidate.entry_allowed:
            self.logger().info(
                f"[MOMENTUM_CANDIDATE_ALLOWED]  {candidate.symbol} | "
                f"score={candidate.score:.1f} | "
                f"1h={candidate.trend_1h:+.1f}% | "
                f"4h={candidate.trend_4h:+.1f}% | "
                f"vol_exp={candidate.volume_expansion:.1f}x | "
                f"rs_vs_btc={candidate.relative_strength:+.1f}% | "
                f"rsi={candidate.rsi:.0f} | "
                f"spread={candidate.spread_pct:.2f}% | "
                f"regime={candidate.regime_at_score}"
            )
            return

        now = self.market_data_provider.time() if self.market_data_provider else self._clock()
        momentum_cfg = getattr(self.config, "momentum_sleeve", {}) or {}
        throttle = float(self._momentum_config_value(
            momentum_cfg,
            "rejected_log_throttle_seconds",
            300,
        ))
        last_logged = self._momentum_rejected_log_ts.get(candidate.symbol, 0.0)
        if now - last_logged < throttle:
            return

        self._momentum_rejected_log_ts[candidate.symbol] = now
        all_reasons = ", ".join(candidate.all_rejection_reasons)
        self.logger().info(
            f"[MOMENTUM_CANDIDATE_REJECTED] {candidate.symbol} | "
            f"score={candidate.score:.1f} | "
            f"primary={candidate.primary_rejection_reason} | "
            f"all=[{all_reasons}] | "
            f"regime={candidate.regime_at_score}"
        )

    def _momentum_spread_pct(self, symbol: str) -> float:
        raw_spread = float(getattr(self, "pair_spreads", {}).get(symbol, 0.0) or 0.0)
        return raw_spread * 100.0 if raw_spread <= 1.0 else raw_spread

    @staticmethod
    def _momentum_config_value(momentum_cfg: Any, key: str, default=None):
        if isinstance(momentum_cfg, dict):
            return momentum_cfg.get(key, default)
        return getattr(momentum_cfg, key, default)

    @classmethod
    def _momentum_config_mode(cls, momentum_cfg: Any) -> str:
        mode = cls._momentum_config_value(momentum_cfg, "mode", "detect_only")
        mode_value = getattr(mode, "value", mode)
        return str(mode_value).lower()

    @staticmethod
    def _momentum_config_as_dict(momentum_cfg: Any) -> dict:
        if isinstance(momentum_cfg, dict):
            return momentum_cfg
        if not momentum_cfg:
            return {}
        ...
        for dump_method in ("model_dump", "dict"):
            if hasattr(momentum_cfg, dump_method):
                try:
                    dumped = getattr(momentum_cfg, dump_method)()
                except Exception:
                    continue
        ...
        try:
            return dict(vars(momentum_cfg))
        except TypeError:
            return {}

    @staticmethod
    def _wick_ratio_to_momentum_risk(wick_ratio: float) -> float:
        try:
            ratio = float(wick_ratio)
        except (TypeError, ValueError):
            ratio = 0.0
        return max(0.0, min(1.0, 1.0 - min(ratio, 1.0)))
    ...
    def _check_smart_entry_v2(self, symbol: str) -> bool:
        ...
        try:
            ...
            # AI-F1: Store indicators for decision logging
            self._last_entry_indicators[symbol] = {
                "rsi": indicators_v2.rsi_14,
                "atr_pct": indicators_v2.atr_pct,
                "wick_ratio": indicators_v2.wick_ratio,
                "change_5m_pct": indicators_v2.change_5m_pct,
                "vwap_deviation_pct": (
                    float((indicators_v2.price - indicators_v2.vwap) / indicators_v2.vwap * 100)
                    if indicators_v2.vwap and indicators_v2.vwap != 0 else None
                ),
            }
    ...
    def _fees_in_base_token(self) -> bool:
        """Return True when the exchange deducts trading fees in the base (received) token.

        Bitget spot deducts fees from the received asset, so a BUY of 100 PEPE
        may only credit 99.9 PEPE.  When ``deduct_base_fees=True`` on the
        GridExecutorConfig the executor accounts for this, preventing
        "Insufficient balance" errors on the subsequent SELL.
        """
        connector_name = self.config.connector_name.lower()
        return "bitget" in connector_name

    def _get_close_cooldown_sec(self, close_type, symbol: str) -> int:
        """Return cooldown duration (seconds) from close_cooldowns config for this close type.

        Reads `close_cooldowns` block from config (previously defined but never consumed).
        Applies `loss_streak_multiplier` when the symbol has consecutive negative roundtrips.
        Falls back to `blacklist_after_timeout_sec` (default 1800s) if no mapping found.
        """
        from hummingbot.strategy_v2.models.executors import CloseType as _CT
        cc = getattr(self.config, 'close_cooldowns', {}) or {}
        if not cc:
            return int(getattr(self.config, 'blacklist_after_timeout_sec', 1800))

        ct_str = str(close_type).upper()
        if close_type in {_CT.NO_PROGRESS_TIMEOUT, _CT.NO_FILL_TIMEOUT,
                          _CT.TIME_LIMIT} or 'NO_PROGRESS' in ct_str or 'TIME_LIMIT' in ct_str:
            base_sec = int(cc.get('no_progress_sec', 3600))
        elif 'HARD_CAP' in ct_str:
            base_sec = int(cc.get('no_progress_sec', 3600))
        elif 'FAILED' in ct_str:
            base_sec = int(cc.get('failed_sec', 21600))
        elif 'STOP_LOSS' in ct_str or 'EARLY_STOP' in ct_str:
            base_sec = int(cc.get('early_stop_sec', 10800))
        else:
            return int(getattr(self.config, 'blacklist_after_timeout_sec', 1800))

        # Apply streak multiplier
        streak = self._loss_streaks.get(symbol, 0)
        if streak > 0:
            multiplier = float(cc.get('loss_streak_multiplier', 1.0))
            max_streak = int(cc.get('max_streak_before_blacklist', 3))
            effective_streak = min(streak, max_streak)
            base_sec = int(base_sec * (multiplier ** effective_streak))
            self.logger().info(
                f"📈 close_cooldowns streak={streak} for {symbol}: "
                f"cooldown={base_sec / 3600:.1f}h (mult={multiplier}^{effective_streak})"
            )
        return base_sec

    def _get_regime_timeouts(self) -> tuple:
        """Return (no_fill_sec, no_progress_sec) from regime_timeouts config if available.

        Falls back to the scalar config values when regime_timeouts is empty or the
        current regime has no entry.  This lets CHOP get a longer no-progress window
        while BEAR gets a shorter one without changing the global scalar values.
        """
        rt = getattr(self.config, 'regime_timeouts', {}) or {}
        regime = (getattr(self, '_last_detected_regime', None) or 'CHOP').upper()

        regime_key_map = {
            'BULL': ('bull_no_fill_sec', 'bull_no_progress_sec'),
            'CHOP': ('chop_no_fill_sec', 'chop_no_progress_sec'),
            'BEAR': ('bear_no_fill_sec', 'bear_no_progress_sec'),
        }
        fill_key, progress_key = regime_key_map.get(regime, ('chop_no_fill_sec', 'chop_no_progress_sec'))

        no_fill_sec = rt.get(fill_key, self.config.no_fill_timeout_sec)
        no_progress_sec = rt.get(progress_key, self.config.no_progress_timeout_sec)
        return int(no_fill_sec), int(no_progress_sec)
    ...
    def _create_grid_action(self, symbol: str, total_amount_quote: Optional[Decimal] = None) -> CreateExecutorAction:
        ...
        # min_range must accommodate num_grids levels: spacing = range / (num_grids-1)
        # so min_range = min_spread × (num_grids - 1). Use max(1, num_grids-1) to avoid /0.
        fee_based_min_range = max(1, num_grids - 1) * min_spread_per_level_pct
        # Also enforce the config-level minimum spacing per level
        _cfg_min_spacing = float(getattr(self.config, 'min_grid_level_spacing_pct', 1.0))
        config_based_min_range = _cfg_min_spacing * max(1, num_grids - 1)
        min_range_pct = max(1.0, fee_based_min_range, config_based_min_range)  # At least 1% or largest floor
        ...
        atr_info = ""
        volatility_pct = 0.0  # Captured for dynamic TP (Tier 3)
        ...
        # Resolve regime-specific timeouts (regime_timeouts config block)
        regime_fill_sec, regime_progress_sec = self._get_regime_timeouts()

        # Story D1: Calculate adaptive timeout based on market volatility
        adaptive_timeout_sec = regime_fill_sec  # Default fallback (regime-aware)
        try:
            ...
            if candle_df is not None and not candle_df.empty and len(candle_df) >= 14:
                ...
                timeout_result = get_recommended_timeout(
                    ...
                    low_prices=lows,
                    close_prices=closes,
                    base_timeout_sec=regime_fill_sec
                )
                ...
                if timeout_result.adjustment_factor != 1.0:
                    self.logger().info(
                        ...
                        f"{adaptive_timeout_sec}s (was {regime_fill_sec}s regime={getattr(self, '_last_detected_regime', 'CHOP')}, "
        ...
        # Use the connector-specific order types from the config schema:
        # Kraken spot -> LIMIT_MAKER, Bitget spot -> LIMIT.
        base_triple_barrier = self.config.triple_barrier_config
        open_order_type = getattr(base_triple_barrier, "open_order_type", OrderType.LIMIT)
        take_profit_order_type = getattr(base_triple_barrier, "take_profit_order_type", open_order_type)

        # Build trailing stop if configured
        trailing_stop = None
        if self.config.trailing_stop_activation_pct and self.config.trailing_stop_delta_pct:
            trailing_stop = TrailingStop(
                activation_price=self.config.trailing_stop_activation_pct,
                trailing_delta=self.config.trailing_stop_delta_pct,
            )
        ...
        grid_config = GridExecutorConfig(
            ...
            # Regime-aware: no_fill and no_progress come from regime_timeouts block
            custom_info={
                "no_fill_timeout_sec": adaptive_timeout_sec,  # D1: Dynamic timeout (regime-aware base)
                "no_progress_timeout_sec": regime_progress_sec,  # Regime-aware
                ...
                "no_progress_max_extension_sec": regime_progress_sec * 2,  # Regime-aware extension
                "rapid_fill_window_sec": self.config.rapid_fill_window_sec,  # Story 2.1/2.2
                "max_hold_time_sec": self.config.max_hold_time_seconds,  # Reuse existing config
                "close_grace_sec": self.config.close_grace_sec,
                "aggressive_close_method": self.config.aggressive_close_method,
                "aggressive_close_slippage_guard_pct": self.config.aggressive_close_slippage_guard_pct,
                "fee_aware_timeout_bypass_sec": self.config.fee_aware_timeout_bypass_sec,
                # Tier 3: Dynamic TP parameters
                "atr_pct": volatility_pct,
                "dynamic_tp_enabled": self.config.dynamic_tp_enabled,
                ...
                "min_grid_profit_pct": float(getattr(self.config, "min_grid_profit_pct", 0.0)),
                "fee_aware_min_net_profit_pct": float(min_net_profit_pct),
        ...
        try:
            ...
            # Reset high water mark for this symbol so a stale peak from the previous
            # executor doesn't immediately trigger PROFIT_LOCK on the new grid.
            if hasattr(self, 'professional_risk_manager'):
                self.professional_risk_manager.position_high_water_marks.pop(symbol, None)

            # Phase 1.1: Track entry price for stop-loss monitoring
            self.entry_prices[symbol] = current_price
            # T1-K4: Persist entry price to SQLite
            if self.entry_price_store:
                try:
                    self.entry_price_store.save(symbol, current_price, executor_id=grid_config.id)
                except Exception as e:
                    self.logger().error(f"T1-K4: Failed to persist entry price for {symbol}: {e}")
    ...
    def _create_stop_action(
        ...
        if executor_info:
            ...
            position_size_quote = _as_decimal(custom_info.get("position_size_quote", Decimal("0")))
            position_size_base = _as_decimal(custom_info.get("position_size_base", Decimal("0")))
            filled_amount = _as_decimal(getattr(executor_info, "filled_amount_quote", Decimal("0")))

            # Also check filled_amount_quote as fallback
            if position_size_base > Decimal("0"):
                has_open_position = True
                self.logger().warning(
                    f"⚠️  Executor has open position: {position_size_base} base - "
            ...
            if has_open_position:
                ...
                if not can_close:
                    self.logger().warning(
                        ...
                        f"   Proposed reason: {stop_reason or 'unknown'}\n"
                        f"   Emergency: {emergency}\n"
                        f"   Action: {'Emergency bypass - attempting close' if emergency else 'Blocked - will not attempt close'}"
                    )
                    if not stop_allowed_after_preclose(can_close=False, emergency=emergency):
                        return None
                else:
        ...
        if self.active_coin:
            self.logger().info(
                f"⏳ Keeping entry/exposure tracking for {self.active_coin} until close fill is confirmed "
                f"(executor_id={self.active_executor_id[:8]}..., reason={stop_reason or 'unknown'})"
            )
        ...
        try:
            ...
            stop_action = StopExecutorAction(
                ...
                early_stop_reason=self._early_stop_reason_from_stop_reason(stop_reason, emergency),
            )
    ...
    def to_format_status(self) -> List[str]:
        ...
        """
        status = []
        # Currency symbol based on quote asset (€ for EUR, $ for USD/USDT)
        qs = {"EUR": "€", "USD": "$", "USDT": "$"}.get(
            getattr(self.config, 'quote_asset', 'EUR'), getattr(self.config, 'quote_asset', '$'))
        ...
        if self.total_exposure > Decimal("0"):
            ...
            status.append(f"║ 📊 Total Exposure: {qs}{self.total_exposure:.2f} ({exposure_pct:.1f}%)              ║")
        ...
        if self.active_coin:
            ...
            if trend:
                ...
                status.append(f"║ Price: {qs}{trend.current_price:8.4f}                                     ║")
                ...
                if entry_price:
                    ...
                    if self.config.stop_loss_pct is not None:
                        ...
                        stop_loss_price = entry_price * (Decimal('1') - self.config.stop_loss_pct)
                        status.append(
                            f"║ Entry: {qs}{entry_price:.4f} | Stop-Loss: {qs}{stop_loss_price:.4f} (-{stop_loss_pct:.2f}%) ║")
                    else:
                        status.append(
                            f"║ Entry: {qs}{entry_price:.4f} | Stop-Loss: DISABLED                 ║")
                    status.append(
                ...
                if self.active_executor_id:
                    ...
                    if executor_info:
                        ...
                        status.append(f"║ {pnl_emoji} P&L: {qs}{pnl_quote:+.2f} ({pnl_pct:+.2f}%) | Fees: {qs}{fees:.2f} ║")
                        if filled > 0:
                            status.append(f"║ 📊 Volume Traded: {qs}{filled:.2f}                              ║")

                        # MONITORING FIX: Log executor P&L to log file so collector can find it
                        self.logger().info(
                            f"📊 MONITORING: Executor P&L: {qs}{pnl_quote:+.2f} ({pnl_pct:+.2f}%) | Active Coin: {self.active_coin or 'None'}")  # noqa: E501
        else:
        ...
        if self.executors_info:
            ...
            if total_volume > 0:
                ...
                status.append(
                    f"║ {total_emoji} Total P&L: {qs}{total_pnl_quote:+.2f} ({total_pnl_pct:+.2f}%) | Fees: {qs}{total_fees:.2f} ║")  # noqa: E501
                status.append(f"║ 📊 Total Volume: {qs}{total_volume:.2f} ({len(self.executors_info)} executor(s)) ║")

                # MONITORING FIX: Log P&L to log file so collector can find it
                self.logger().info(
                    f"📊 MONITORING: Total P&L: {qs}{
                        total_pnl_quote:+.2f} ({
                        total_pnl_pct:+.2f}%) | Active Coin: {
                        self.active_coin or 'None'} | Exposure: {qs}{
        ...
        num_monitored = len(self.monitored_coins) if hasattr(self, 'monitored_coins') else 0
        ...
        if num_price_histories > 0 or num_executors > 0:
            ...
            # Warn only when price histories exceed both monitored + active + buffer.
            # price_history_for_volatility is kept for all monitored coins (not just active
            # grids), so comparing only to active_coins causes false alarms when there are
            # monitored candidates but no open executors.
            if num_price_histories > max(num_active_coins, num_monitored) + 5:
    ...
    def _check_multi_timeframe_buy_conditions(self, coin: str) -> bool:
        ...
        try:
            ...
            # ============================================================
            quick_filter_threshold = float(getattr(self.config, 'trend_min_change_pct', 0.5) or 0.5)
            validation = self.trend_calculator.validate_trend(
                coin,
                min_trend_threshold=quick_filter_threshold,
            )
            if not validation:
            ...
            if not validation.passes:
                reason_map = {
                    ...
                    TrendStatus.BEARISH: (
                        f"bearish trend ({validation.trend_score_pct:+.2f}% "
                        f"< -{quick_filter_threshold:.3f}%)"
                    ),
                    TrendStatus.SIDEWAYS: (
                        f"sideways trend ({validation.trend_score_pct:+.2f}% "
                        f"< +{quick_filter_threshold:.3f}%)"
                    ),
                }
            ...
            # Downtrend gate: block entries during short/mid/long downtrends
            downtrend_gate_enabled = getattr(self.config, "downtrend_gate_enabled", True)
            if downtrend_gate_enabled and hasattr(trend, "trend_60m"):
                downtrend_1h_min = getattr(self.config, "downtrend_gate_1h_min", -1.0)
                downtrend_4h_min = getattr(self.config, "downtrend_gate_4h_min", -0.75)
                downtrend_24h_min = getattr(self.config, "downtrend_gate_24h_min", -0.5)
                if (trend.trend_60m < downtrend_1h_min or
                        trend.trend_240m < downtrend_4h_min or
                        trend.trend_1440m < downtrend_24h_min):
                    self.logger().info(
                        f"[DECISION] ❌ {coin} BUY REJECTED (Downtrend Gate): "
                        f"1h {trend.trend_60m:+.2f}% < {downtrend_1h_min:+.2f}% or "
                        f"4h {trend.trend_240m:+.2f}% < {downtrend_4h_min:+.2f}% or "
                        f"24h {trend.trend_1440m:+.2f}% < {downtrend_24h_min:+.2f}%"
                    )
                    return False
            ...
            if hasattr(trend, 'long_trend_warmup') and trend.long_trend_warmup:
                ...
                if warmup_240m_ok and warmup_60m_ok and both_positive:
                    # Warmup max-24h-trend guard: don't buy already-extended coins during warmup.
                    # Normal SmartEntryFilter is bypassed by the warmup return-True, so we enforce
                    # an explicit cap here. Uses warmup_max_trend_24h_pct config (default 8.0%).
                    _warmup_max_24h = getattr(self.config, 'warmup_max_trend_24h_pct', 8.0)
                    if trend.trend_1440m > _warmup_max_24h:
                        self.logger().warning(
                            f"[DECISION] ❌ {coin} BUY REJECTED (warm-up mode - 24H EXTENDED): "
                            f"24h trend ({trend.trend_1440m:+.2f}%) > warmup_max_trend_24h_pct "
                            f"({_warmup_max_24h:.1f}%) — coin already extended, skip during warmup"
                        )
                        return False
    ...
    def should_exit_position(self, coin: str) -> Optional[str]:
        ...
        try:
            ...
            if max_hold_time > 0 and time_since_switch >= max_hold_time:
                ...
                executor_info = self._get_executor_info(self.active_executor_id) if self.active_executor_id else None
                if not self._fee_aware_non_emergency_exit_allowed(
                    coin,
                    executor_info,
                    current_price,
                    "max_hold_time_exit",
            ...
            net_price_change_pct = price_change_pct - estimated_fees_pct
            ...
            if hasattr(trend, 'trend_60m') and trend.trend_60m != 0.0:
                ...
                # NO-LOSS EXIT: Exit on trend weakness once net P&L covers fees
                no_loss_exit_enabled = getattr(self.config, "no_loss_exit_enabled", True)
                no_loss_exit_min_net_pct = getattr(self.config, "no_loss_exit_min_net_pct", 0.0)
                if no_loss_exit_enabled and net_price_change_pct >= no_loss_exit_min_net_pct:
                    if severe_short_trend or trend.trend_240m < exit_mid_threshold:
                        if in_hard_grace_period:
                            self.logger().info(
                                f"⏸️  {coin} NO-LOSS EXIT BLOCKED: Hard minimum hold time not met "
                                f"({time_since_switch / 60:.1f} min < {hard_min_hold_time / 60:.1f} min)"
                            )
                            return None
                        self.logger().info(
                            f"[EXIT] 🟢 {coin} NO-LOSS EXIT TRIGGERED:\n"
                            f"   Net Price Change: {net_price_change_pct:+.2f}% (fees est: {estimated_fees_pct:.2f}%)\n"
                            f"   [TREND] 1h: {trend.trend_60m:+.2f}% | 4h: {trend.trend_240m:+.2f}%\n"
                            f"   [REASON] Trend weakening while breakeven+fees achieved"
                        )
                        executor_info = self._get_executor_info(self.active_executor_id) if self.active_executor_id else None
                        if not self._fee_aware_non_emergency_exit_allowed(
                            coin,
    ...
    async def _detect_current_regime(self):
        ...
        try:
            ...
            sample_pair = None
            sample_source = None
            sample_trend_obj = None

            # Prefer macro inputs directly from trend data. Do not add BTC/SOL to
            # monitored_coins; this is only a read-only regime sample selection.
            for coin, direct_source, cached_source in (
                ("BTC", "btc_direct", "btc_cached"),
                ("SOL", "sol_direct", "sol_cached"),
            ):
                test_pair = f"{coin}-{quote}"
                if test_pair in trend_map:
                    sample_pair = test_pair
                    sample_source = direct_source
                    sample_trend_obj = trend_map[test_pair]
                    break

                cached_trend = getattr(self, "_regime_macro_trends", {}).get(test_pair)
                if cached_trend is not None:
                    sample_pair = test_pair
                    sample_source = cached_source
                    sample_trend_obj = cached_trend
                    break
            ...
            if not sample_pair:
                ...
                # Fallback: use first monitored coin with trend data, preserving
                # the existing altcoin-based behavior when macro inputs are absent.
                for pair in sample_pairs:
                    if pair in trend_map:
                        sample_pair = pair
                        sample_source = "monitored_fallback"
                        sample_trend_obj = trend_map[pair]
                        break

            if not sample_pair or sample_trend_obj is None:
                return None

            self._last_regime_input_pair = sample_pair
            self._last_regime_input_source = sample_source
            self.logger().info(
                f"🌡️ REGIME input selected: source={sample_source} pair={sample_pair}"
            )

            trend_obj = sample_trend_obj
    ...
    def _apply_adaptive_filters(self, filters: Dict[str, any]) -> None:
        ...
        try:
            ...
            filter_mapping = {
                ...
                # Trend filters — regime-aware: BULL loosens, BEAR tightens
                'max_trend_24h_pct': 'max_trend_24h_pct',  # Direct match ✅
                'max_trend_4h_pct': 'max_trend_4h_pct',    # Direct match ✅
            ...
            max_fields = {  # These are ceilings—adaptive can go LOWER but not HIGHER
                # NOTE: rsi_buy_max intentionally excluded — BULL regime must raise it above
                # the YAML baseline (e.g. baseline=72, BULL=76), so we allow free movement.
                # The per-regime YAML values already express the operator's intent.
                'vwap_max_deviation_pct', 'max_atr_pct_for_grid',
                'max_5m_spike_pct', 'max_up_accel_pct', 'max_trend_24h_pct',
                'max_trend_4h_pct',
            }
            min_fields = {  # These are floors—adaptive can go HIGHER but not LOWER
                # NOTE: min_atr_pct_for_grid is intentionally excluded here so that
                # the regime (BULL/CHOP/BEAR) can freely lower it below the YAML base.
                # BULL=0.05, CHOP=0.06, BEAR=0.08 are all valid regime overrides.
                'rsi_extreme_low', 'min_wick_ratio',
            }
